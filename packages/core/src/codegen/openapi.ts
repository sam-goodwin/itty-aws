...
export interface OpenApiConvertOptions {
   ...
   * left alone.
   *
   * - `"as-is"` (default): `PascalCase(operationId)` (`Apps_list` → `AppsList`)
   * - `"verbNoun"`: go-swagger `Resource_action` → `listApps` / `getApp`
   *   (`show` → `get`). Already-camelCase ids are unchanged.
   *
    ...
    for (const method of httpMethods) {
      ...
      const named =
        (options.operationNames !== undefined
          ...
          : undefined) ??
        (options.operationNaming === "verbNoun"
