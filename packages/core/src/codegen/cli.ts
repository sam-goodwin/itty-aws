  ...
  const command = Command.make(
    ...
    (config) =>
      Effect.gen(function* () {
        ...
        //
        // A FILTERED run (`--resource`/`--limit`) only knows about the
        // resources it generated, so rewriting the barrel from those would
        // delete every other export — which is how #397 shipped a cloudflare
        // barrel exporting `workers` and nothing else, silently dropping 119
        // services. Merge into the existing barrel instead, so a filtered run
        // can still ADD a brand-new resource without removing anything.
        const barrelPath = path.join(outDir, "index.ts");
        const filtered = config.resource !== "" || config.limit > 0;
