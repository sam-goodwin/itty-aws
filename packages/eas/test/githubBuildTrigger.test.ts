...
import { githubBuildTrigger } from "../src/operations/githubBuildTrigger.ts";
...
// without the privilege to invoke githubBuildTrigger mutations and emits
...
describe("githubBuildTrigger", () => {
  it(
    "happy path - executes the githubBuildTrigger mutation and returns a result",
    async () => {
      const result = await runEffect(githubBuildTrigger({}));
  ...
  it(
    ...
    async () => {
      // The githubBuildTrigger mutation wrapper has no input variables, so
      ...
      const error = await runEffect(githubBuildTrigger({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage GitHub build triggers",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the githubBuildTrigger mutation
