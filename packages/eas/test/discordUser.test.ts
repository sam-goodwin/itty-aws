...
import { discordUser } from "../src/operations/discordUser.ts";
...
// without the privilege to invoke discordUser mutations and emits
...
describe("discordUser", () => {
  it(
    "happy path - executes the discordUser mutation and returns a result",
    async () => {
      const result = await runEffect(discordUser({}));
  ...
  it(
    ...
    async () => {
      // The discordUser mutation wrapper has no input variables, so the
      ...
      const error = await runEffect(discordUser({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage Discord user links",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the discordUser mutation wrapper is
