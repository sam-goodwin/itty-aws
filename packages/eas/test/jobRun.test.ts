...
import { jobRun } from "../src/operations/jobRun.ts";
...
describe("jobRun", () => {
  it("happy path - executes the jobRun mutation and returns a result", async () => {
    const result = await runEffect(jobRun({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(jobRun({}).pipe(Effect.flip));
