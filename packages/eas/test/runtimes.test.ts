...
import { runtimes } from "../src/operations/runtimes.ts";
...
describe("runtimes", () => {
  it("happy path - executes the runtimes query and returns a result", async () => {
    const result = await runEffect(runtimes({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(runtimes({}).pipe(Effect.flip));
