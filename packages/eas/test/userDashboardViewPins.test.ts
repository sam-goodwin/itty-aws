...
import { userDashboardViewPins } from "../src/operations/userDashboardViewPins.ts";
...
describe("userDashboardViewPins", () => {
  it("happy path - executes the userDashboardViewPins mutation and returns a result", async () => {
    const result = await runEffect(userDashboardViewPins({}));
    expect(result).toBeDefined();
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      userDashboardViewPins({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(userDashboardViewPins({}).pipe(Effect.flip));
