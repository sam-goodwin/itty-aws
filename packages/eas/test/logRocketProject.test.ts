...
import { logRocketProject } from "../src/operations/logRocketProject.ts";
...
describe("logRocketProject", () => {
  it("happy path - executes the logRocketProject mutation and returns a result", async () => {
    const result = await runEffect(logRocketProject({}));
    expect(result).toBeDefined();
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      logRocketProject({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(logRocketProject({}).pipe(Effect.flip));
