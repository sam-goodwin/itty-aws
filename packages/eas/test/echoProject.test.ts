...
import { echoProject } from "../src/operations/echoProject.ts";
...
// without the privilege to invoke echoProject mutations and emits
...
describe("echoProject", () => {
  it(
    "happy path - executes the echoProject mutation and returns a result",
    async () => {
      const result = await runEffect(echoProject({}));
  ...
  it(
    ...
    async () => {
      // The echoProject mutation wrapper has no input variables, so the
      ...
      const error = await runEffect(echoProject({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage Echo projects",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the echoProject mutation wrapper is
