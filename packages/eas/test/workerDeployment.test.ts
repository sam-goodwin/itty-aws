...
import { workerDeployment } from "../src/operations/workerDeployment.ts";
...
describe("workerDeployment", () => {
  it("happy path - executes the workerDeployment query and returns a result", async () => {
    const result = await runEffect(workerDeployment({}));
    expect(result).toBeDefined();
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      workerDeployment({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(workerDeployment({}).pipe(Effect.flip));
    expect(error._tag).toBe("EasValidationError");
  }, { timeout: 30_000 });

  it("error - EasAppNoDevDomainName when the app has no dev domain name configured", async () => {
    const error = await runEffect(workerDeployment({}).pipe(Effect.flip));
    expect(error._tag).toBe("EasAppNoDevDomainName");
  }, { timeout: 30_000 });

  it("error - EasDevDomainNameTaken when the requested dev domain name is already taken", async () => {
    const error = await runEffect(workerDeployment({}).pipe(Effect.flip));
