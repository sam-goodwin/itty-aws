...
import { userAuditLog } from "../src/operations/userAuditLog.ts";
...
describe("userAuditLog", () => {
  it("happy path - executes the userAuditLog mutation and returns a result", async () => {
    const result = await runEffect(userAuditLog({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(userAuditLog({}).pipe(Effect.flip));
