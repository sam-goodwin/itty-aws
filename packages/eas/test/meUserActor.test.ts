...
import { meUserActor } from "../src/operations/meUserActor.ts";
...
describe("meUserActor", () => {
  it("happy path - executes the meUserActor query and returns a result", async () => {
    const result = await runEffect(meUserActor({}));
    if (result !== null) {
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(meUserActor({}).pipe(Effect.flip));
