...
import { statuspageService } from "../src/operations/statuspageService.ts";
...
describe("statuspageService", () => {
  it("happy path - executes the statuspageService query and returns a result", async () => {
    const result = await runEffect(statuspageService({}));
    expect(result).toBeDefined();
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      statuspageService({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(statuspageService({}).pipe(Effect.flip));
