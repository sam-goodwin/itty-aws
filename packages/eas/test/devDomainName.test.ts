...
import { devDomainName } from "../src/operations/devDomainName.ts";
...
// without the privilege to invoke devDomainName mutations and emits
...
describe("devDomainName", () => {
  it(
    "happy path - executes the devDomainName mutation and returns a result",
    async () => {
      const result = await runEffect(devDomainName({}));
  ...
  it(
    ...
    async () => {
      // The devDomainName mutation wrapper has no input variables, so the
      ...
      const error = await runEffect(devDomainName({}).pipe(Effect.flip));
  ...
  it(
    ...
    async () => {
      // devDomainName mutations that operate on an app without an existing
      // dev domain name (e.g. unassign/rename without a prior assignment)
      // are rejected with extensions.errorCode = APP_NO_DEV_DOMAIN_NAME.
      const error = await runEffect(devDomainName({}).pipe(Effect.flip));
  ...
  it(
    ...
    async () => {
      // devDomainName mutations that target a dev domain name already
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage dev domain names",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the devDomainName mutation wrapper is
