...
import { echoTurn } from "../src/operations/echoTurn.ts";
...
// without the privilege to invoke echoTurn mutations and emits
...
describe("echoTurn", () => {
  it(
    "happy path - executes the echoTurn mutation and returns a result",
    async () => {
      const result = await runEffect(echoTurn({}));
  ...
  it(
    ...
    async () => {
      // The echoTurn mutation wrapper has no input variables, so the only
      ...
      const error = await runEffect(echoTurn({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage Echo turns",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the echoTurn mutation wrapper is
