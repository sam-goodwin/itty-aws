...
import { echoMessagePart } from "../src/operations/echoMessagePart.ts";
...
// without the privilege to invoke echoMessagePart mutations and emits
...
describe("echoMessagePart", () => {
  it(
    "happy path - executes the echoMessagePart mutation and returns a result",
    async () => {
      const result = await runEffect(echoMessagePart({}));
  ...
  it(
    ...
    async () => {
      // The echoMessagePart mutation wrapper has no input variables, so the
      ...
      const error = await runEffect(echoMessagePart({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage Echo message parts",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the echoMessagePart mutation wrapper
