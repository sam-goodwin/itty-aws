...
import { appStoreConnectApiKey } from "../src/operations/appStoreConnectApiKey.ts";
...
// without the privilege to invoke App Store Connect API key mutations and
...
describe("appStoreConnectApiKey", () => {
  it(
    "happy path - executes the appStoreConnectApiKey mutation and returns a result",
    async () => {
      const result = await runEffect(appStoreConnectApiKey({}));
  ...
  it(
    ...
    async () => {
      // The appStoreConnectApiKey mutation wrapper has no input variables, so
      ...
      const error = await runEffect(
        appStoreConnectApiKey({}).pipe(Effect.flip),
      );
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage App Store Connect API keys",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the appStoreConnectApiKey mutation
