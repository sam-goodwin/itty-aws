...
import { workflowsInsights } from "../src/operations/workflowsInsights.ts";
...
describe("workflowsInsights", () => {
  it("happy path - executes the workflowsInsights mutation and returns a result", async () => {
    const result = await runEffect(workflowsInsights({}));
    expect(result).toBeDefined();
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      workflowsInsights({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(workflowsInsights({}).pipe(Effect.flip));
