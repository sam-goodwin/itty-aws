...
import { updateBranch } from "../src/operations/updateBranch.ts";
...
describe("updateBranch", () => {
  it("happy path - executes the updateBranch mutation and returns a result", async () => {
    const result = await runEffect(updateBranch({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(updateBranch({}).pipe(Effect.flip));
