...
import { appleAppIdentifier } from "../src/operations/appleAppIdentifier.ts";
...
// without the privilege to invoke Apple app identifier mutations and emits
...
describe("appleAppIdentifier", () => {
  it(
    "happy path - executes the appleAppIdentifier mutation and returns a result",
    async () => {
      const result = await runEffect(appleAppIdentifier({}));
  ...
  it(
    ...
    async () => {
      // The appleAppIdentifier mutation wrapper has no input variables, so
      ...
      const error = await runEffect(
        appleAppIdentifier({}).pipe(Effect.flip),
      );
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage Apple app identifiers",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the appleAppIdentifier mutation
