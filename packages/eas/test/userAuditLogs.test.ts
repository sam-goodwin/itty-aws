...
import { userAuditLogs } from "../src/operations/userAuditLogs.ts";
...
describe("userAuditLogs", () => {
  it("happy path - executes the userAuditLogs query and returns a result", async () => {
    const result = await runEffect(userAuditLogs({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(userAuditLogs({}).pipe(Effect.flip));
