...
import { buildAnnotations } from "../src/operations/buildAnnotations.ts";
...
// without the privilege to invoke buildAnnotations queries and emits
...
describe("buildAnnotations", () => {
  it(
    "happy path - executes the buildAnnotations query and returns a result",
    async () => {
      const result = await runEffect(buildAnnotations({}));
  ...
  it(
    ...
    async () => {
      // The buildAnnotations query wrapper has no input variables, so the
      ...
      const error = await runEffect(buildAnnotations({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to read build annotations",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the buildAnnotations query wrapper is
