...
import { deviceRunSessions } from "../src/operations/deviceRunSessions.ts";
...
// without the privilege to invoke deviceRunSessions queries and emits
...
describe("deviceRunSessions", () => {
  it(
    "happy path - executes the deviceRunSessions query and returns a result",
    async () => {
      const result = await runEffect(deviceRunSessions({}));
  ...
  it(
    ...
    async () => {
      // The deviceRunSessions query wrapper has no input variables, so the
      ...
      const error = await runEffect(deviceRunSessions({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to list device run sessions",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the deviceRunSessions query wrapper
