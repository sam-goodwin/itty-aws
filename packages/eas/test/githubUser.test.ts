...
import { githubUser } from "../src/operations/githubUser.ts";
...
// without the privilege to invoke githubUser mutations and emits
...
describe("githubUser", () => {
  it(
    "happy path - executes the githubUser mutation and returns a generateGitHubUserAccessToken field",
    async () => {
      const result = await runEffect(githubUser({}));
      expect(result).toBeDefined();
      // Field is nullable per the SelectionSet schema.
      expect(
        result.generateGitHubUserAccessToken === null ||
          typeof result.generateGitHubUserAccessToken === "string",
      ).toBe(true);
    },
    { timeout: 30_000 },
  );
  ...
  it(
    ...
    async () => {
      // The githubUser mutation wrapper has no input variables, so the
      ...
      const error = await runEffect(githubUser({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage GitHub users",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the githubUser mutation wrapper is
