...
import { appleDeviceRegistrationRequest } from "../src/operations/appleDeviceRegistrationRequest.ts";
...
// without the privilege to invoke Apple device registration request mutations
// and emits extensions.errorCode = UNAUTHORIZED_ERROR (→ EasUnauthorizedOperation).
...
describe("appleDeviceRegistrationRequest", () => {
  it(
    "happy path - executes the appleDeviceRegistrationRequest mutation and returns a result",
    async () => {
      const result = await runEffect(appleDeviceRegistrationRequest({}));
  ...
  it(
    ...
    async () => {
      const error = await Effect.runPromise(
        appleDeviceRegistrationRequest({}).pipe(
          Effect.flip,
  ...
  it(
    ...
    async () => {
      // The appleDeviceRegistrationRequest mutation wrapper has no input
      ...
      const error = await runEffect(
        appleDeviceRegistrationRequest({}).pipe(Effect.flip),
      );
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage Apple device registration requests",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the appleDeviceRegistrationRequest
