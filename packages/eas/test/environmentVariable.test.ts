...
import { environmentVariable } from "../src/operations/environmentVariable.ts";
...
// without the privilege to invoke environmentVariable mutations and emits
...
describe("environmentVariable", () => {
  it(
    "happy path - executes the environmentVariable mutation and returns a result",
    async () => {
      const result = await runEffect(environmentVariable({}));
  ...
  it(
    ...
    async () => {
      // The environmentVariable mutation wrapper has no input variables, so
      ...
      const error = await runEffect(environmentVariable({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage environment variables",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the environmentVariable mutation
