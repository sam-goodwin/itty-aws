...
import { meActor } from "../src/operations/meActor.ts";
...
describe("meActor", () => {
  it("happy path - executes the meActor query and returns a result", async () => {
    const result = await runEffect(meActor({}));
    if (result !== null) {
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(meActor({}).pipe(Effect.flip));
