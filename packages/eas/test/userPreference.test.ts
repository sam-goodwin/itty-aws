...
import { userPreference } from "../src/operations/userPreference.ts";
...
describe("userPreference", () => {
  it("happy path - executes the userPreference mutation and returns a result", async () => {
    const result = await runEffect(userPreference({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(userPreference({}).pipe(Effect.flip));
