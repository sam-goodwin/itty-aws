...
import { deviceRunSession } from "../src/operations/deviceRunSession.ts";
...
// without the privilege to invoke deviceRunSession mutations and emits
...
describe("deviceRunSession", () => {
  it(
    "happy path - executes the deviceRunSession mutation and returns a result",
    async () => {
      const result = await runEffect(deviceRunSession({}));
  ...
  it(
    ...
    async () => {
      // The deviceRunSession mutation wrapper has no input variables, so
      // the only server-side rejection path is GraphQL document validation.
      // The EAS backend emits extensions.errorCode = VALIDATION_ERROR for
      // any server-side schema/argument validation failure on this operation.
      const error = await runEffect(deviceRunSession({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage device run sessions",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the deviceRunSession mutation wrapper
      // is rejected with extensions.errorCode = UNAUTHORIZED_ERROR — distinct
