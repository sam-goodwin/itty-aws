...
import { workflowRuns } from "../src/operations/workflowRuns.ts";
...
describe("workflowRuns", () => {
  it("happy path - executes the workflowRuns query and returns a result", async () => {
    const result = await runEffect(workflowRuns({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(workflowRuns({}).pipe(Effect.flip));
