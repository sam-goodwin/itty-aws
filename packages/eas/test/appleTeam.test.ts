...
import { appleTeam } from "../src/operations/appleTeam.ts";
...
// without the privilege to invoke Apple team mutations and emits
...
describe("appleTeam", () => {
  it(
    "happy path - executes the appleTeam mutation and returns a result",
    async () => {
      const result = await runEffect(appleTeam({}));
  ...
  it(
    ...
    async () => {
      // The appleTeam mutation wrapper has no input variables, so the only
      ...
      const error = await runEffect(appleTeam({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage Apple teams",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the appleTeam mutation wrapper is
