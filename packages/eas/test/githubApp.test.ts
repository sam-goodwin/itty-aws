...
import { githubApp } from "../src/operations/githubApp.ts";
...
// without the privilege to invoke githubApp mutations and emits
...
describe("githubApp", () => {
  it(
    "happy path - executes the githubApp mutation and returns a result",
    async () => {
      const result = await runEffect(githubApp({}));
  ...
  it(
    ...
    async () => {
      // The githubApp mutation wrapper has no input variables, so the only
      ...
      const error = await runEffect(githubApp({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to use the GitHub App",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the githubApp mutation wrapper is
