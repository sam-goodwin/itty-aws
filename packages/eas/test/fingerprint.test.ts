...
import { fingerprint } from "../src/operations/fingerprint.ts";
...
// without the privilege to invoke fingerprint mutations and emits
...
describe("fingerprint", () => {
  it(
    "happy path - executes the fingerprint mutation and returns a result",
    async () => {
      const result = await runEffect(fingerprint({}));
  ...
  it(
    ...
    async () => {
      // The fingerprint mutation wrapper has no input variables, so the
      ...
      const error = await runEffect(fingerprint({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage fingerprints",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the fingerprint mutation wrapper is
