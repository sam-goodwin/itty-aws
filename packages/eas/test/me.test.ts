...
describe("me", () => {
  it("happy path - executes the me mutation and returns a result", async () => {
    const result = await runEffect(me({}));
    expect(result).toBeDefined();
    expect(typeof result.purgeUnfinishedSecondFactorAuthentication.success).toBe("boolean");
    expect(typeof result.scheduleCurrentUserDeletion.id).toBe("string");
  }, { timeout: 30_000 });
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(me({}).pipe(Effect.flip));
