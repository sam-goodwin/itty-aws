...
import { echoMessage } from "../src/operations/echoMessage.ts";
...
// without the privilege to invoke echoMessage mutations and emits
...
describe("echoMessage", () => {
  it(
    "happy path - executes the echoMessage mutation and returns a result",
    async () => {
      const result = await runEffect(echoMessage({}));
  ...
  it(
    ...
    async () => {
      // The echoMessage mutation wrapper has no input variables, so the
      ...
      const error = await runEffect(echoMessage({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage Echo messages",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the echoMessage mutation wrapper is
