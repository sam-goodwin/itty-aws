...
import { userAppPins } from "../src/operations/userAppPins.ts";
...
describe("userAppPins", () => {
  it("happy path - executes the userAppPins mutation and returns a result", async () => {
    const result = await runEffect(userAppPins({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(userAppPins({}).pipe(Effect.flip));
