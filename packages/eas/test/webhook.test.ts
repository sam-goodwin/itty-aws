...
import { webhook } from "../src/operations/webhook.ts";
...
describe("webhook", () => {
  it("happy path - executes the webhook mutation and returns a result", async () => {
    const result = await runEffect(webhook({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(webhook({}).pipe(Effect.flip));
