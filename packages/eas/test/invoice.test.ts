...
import { invoice } from "../src/operations/invoice.ts";
...
// without the privilege to invoke invoice queries and emits
...
describe("invoice", () => {
  it(
    "happy path - executes the invoice query and returns a result",
    async () => {
      const result = await runEffect(invoice({}));
  ...
  it(
    ...
    async () => {
      // The invoice query wrapper has no input variables, so the only
      ...
      const error = await runEffect(invoice({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to read invoices",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the invoice query wrapper is rejected
