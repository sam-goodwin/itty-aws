...
import { androidKeystore } from "../src/operations/androidKeystore.ts";
...
// without the privilege to invoke keystore mutations and emits
...
describe("androidKeystore", () => {
  it(
    "happy path - executes the androidKeystore mutation and returns a result",
    async () => {
      const result = await runEffect(androidKeystore({}));
  ...
  it(
    ...
    async () => {
      // The androidKeystore mutation wrapper has no input variables, so the
      ...
      const error = await runEffect(androidKeystore({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage keystores",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the androidKeystore mutation wrapper
