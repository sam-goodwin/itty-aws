...
import { echoVersion } from "../src/operations/echoVersion.ts";
...
// without the privilege to invoke echoVersion mutations and emits
...
describe("echoVersion", () => {
  it(
    "happy path - executes the echoVersion mutation and returns a result",
    async () => {
      const result = await runEffect(echoVersion({}));
  ...
  it(
    ...
    async () => {
      // The echoVersion mutation wrapper has no input variables, so the
      ...
      const error = await runEffect(echoVersion({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage Echo versions",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the echoVersion mutation wrapper is
