...
// without the privilege to invoke auditLog mutations and emits
...
describe("auditLog", () => {
  it(
    "happy path - executes the auditLog mutation and returns a result",
    async () => {
      const result = await runEffect(auditLog({}));
  ...
  it(
    ...
    async () => {
      // The auditLog mutation wrapper has no input variables, so the only
      ...
      const error = await runEffect(auditLog({}).pipe(Effect.flip));
  ...
  it(
    ...
    async () => {
      ...
      // role/grant required to invoke the auditLog mutation wrapper is
