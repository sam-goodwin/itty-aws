...
import { convexTeamConnection } from "../src/operations/convexTeamConnection.ts";
...
// without the privilege to invoke convexTeamConnection mutations and emits
...
describe("convexTeamConnection", () => {
  it(
    "happy path - executes the convexTeamConnection mutation and returns a result",
    async () => {
      const result = await runEffect(convexTeamConnection({}));
  ...
  it(
    ...
    async () => {
      // The convexTeamConnection mutation wrapper has no input variables,
      // so the only server-side rejection path is GraphQL document
      ...
      const error = await runEffect(
        convexTeamConnection({}).pipe(Effect.flip),
      );
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage the Convex team connection",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the convexTeamConnection mutation
