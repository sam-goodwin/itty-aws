...
import { backgroundJobReceipt } from "../src/operations/backgroundJobReceipt.ts";
...
// without the privilege to invoke backgroundJobReceipt queries and emits
...
describe("backgroundJobReceipt", () => {
  it(
    "happy path - executes the backgroundJobReceipt query and returns a result",
    async () => {
      const result = await runEffect(backgroundJobReceipt({}));
  ...
  it(
    ...
    async () => {
      // The backgroundJobReceipt query wrapper has no input variables, so
      ...
      const error = await runEffect(backgroundJobReceipt({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to read background job receipts",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the backgroundJobReceipt query wrapper
