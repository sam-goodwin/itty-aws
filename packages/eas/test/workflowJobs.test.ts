...
import { workflowJobs } from "../src/operations/workflowJobs.ts";
...
describe("workflowJobs", () => {
  it("happy path - executes the workflowJobs query and returns a result", async () => {
    const result = await runEffect(workflowJobs({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(workflowJobs({}).pipe(Effect.flip));
