...
import { workflowDeviceTestCaseResult } from "../src/operations/workflowDeviceTestCaseResult.ts";
...
describe("workflowDeviceTestCaseResult", () => {
  it("happy path - executes the workflowDeviceTestCaseResult mutation and returns a result", async () => {
    const result = await runEffect(workflowDeviceTestCaseResult({}));
    expect(result).toBeDefined();
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      workflowDeviceTestCaseResult({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(workflowDeviceTestCaseResult({}).pipe(Effect.flip));
