...
import { appleDevice } from "../src/operations/appleDevice.ts";
...
// without the privilege to invoke Apple device mutations and emits
...
describe("appleDevice", () => {
  it(
    "happy path - executes the appleDevice mutation and returns a result",
    async () => {
      const result = await runEffect(appleDevice({}));
  ...
  it(
    ...
    async () => {
      // The appleDevice mutation wrapper has no input variables, so the only
      ...
      const error = await runEffect(appleDevice({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage Apple devices",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the appleDevice mutation wrapper is
