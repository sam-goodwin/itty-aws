...
import { workflowRun } from "../src/operations/workflowRun.ts";
...
describe("workflowRun", () => {
  it("happy path - executes the workflowRun mutation and returns a result", async () => {
    const result = await runEffect(workflowRun({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(workflowRun({}).pipe(Effect.flip));
