...
import { branches } from "../src/operations/branches.ts";
...
// without the privilege to invoke branches queries and emits
...
describe("branches", () => {
  it(
    "happy path - executes the branches query and returns a result",
    async () => {
      const result = await runEffect(branches({}));
  ...
  it(
    ...
    async () => {
      // The branches query wrapper has no input variables, so the only
      ...
      const error = await runEffect(branches({}).pipe(Effect.flip));
  ...
  it(
    ...
    async () => {
      // The branches query is implicitly scoped by the calling actor's
      // current experience. When that resolution fails (the experience has
      // been deleted, the actor is no longer associated with it, etc.),
      // the EAS backend emits extensions.errorCode = EXPERIENCE_NOT_FOUND.
      const error = await runEffect(branches({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to list branches",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the branches query wrapper is rejected
