...
import { uploadSession } from "../src/operations/uploadSession.ts";
...
describe("uploadSession", () => {
  it("happy path - executes the uploadSession mutation and returns a result", async () => {
    const result = await runEffect(uploadSession({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(uploadSession({}).pipe(Effect.flip));
