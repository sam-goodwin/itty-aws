...
import { userInvitation } from "../src/operations/userInvitation.ts";
...
describe("userInvitation", () => {
  it("happy path - executes the userInvitation mutation and returns a result", async () => {
    const result = await runEffect(userInvitation({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(userInvitation({}).pipe(Effect.flip));
