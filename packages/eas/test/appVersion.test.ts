...
import { appVersion } from "../src/operations/appVersion.ts";
...
// without the privilege to invoke appVersion mutations and emits
...
describe("appVersion", () => {
  it(
    "happy path - executes the appVersion mutation and returns a result",
    async () => {
      const result = await runEffect(appVersion({}));
  ...
  it(
    ...
    async () => {
      // The appVersion mutation wrapper has no input variables, so the only
      ...
      const error = await runEffect(appVersion({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage app versions",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the appVersion mutation wrapper is
