...
import { vexoAccountConnection } from "../src/operations/vexoAccountConnection.ts";
...
describe("vexoAccountConnection", () => {
  it("happy path - executes the vexoAccountConnection mutation and returns a result", async () => {
    const result = await runEffect(vexoAccountConnection({}));
    expect(result).toBeDefined();
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      vexoAccountConnection({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(vexoAccountConnection({}).pipe(Effect.flip));
