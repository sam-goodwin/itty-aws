...
import { androidAppBuildCredentials } from "../src/operations/androidAppBuildCredentials.ts";
...
// without the privilege to invoke Android build credential mutations and emits
...
describe("androidAppBuildCredentials", () => {
  it(
    "happy path - executes the androidAppBuildCredentials mutation and returns a result",
    async () => {
      const result = await runEffect(androidAppBuildCredentials({}));
  ...
  it(
    ...
    async () => {
      const error = await Effect.runPromise(
        androidAppBuildCredentials({}).pipe(
          Effect.flip,
  ...
  it(
    ...
    async () => {
      // The androidAppBuildCredentials mutation wrapper has no input
      ...
      const error = await runEffect(
        androidAppBuildCredentials({}).pipe(Effect.flip),
      );
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage Android build credentials",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the androidAppBuildCredentials mutation
