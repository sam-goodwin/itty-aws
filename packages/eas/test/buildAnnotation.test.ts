...
import { buildAnnotation } from "../src/operations/buildAnnotation.ts";
...
// without the privilege to invoke buildAnnotation mutations and emits
...
describe("buildAnnotation", () => {
  it(
    "happy path - executes the buildAnnotation mutation and returns a result",
    async () => {
      const result = await runEffect(buildAnnotation({}));
  ...
  it(
    ...
    async () => {
      // The buildAnnotation mutation wrapper has no input variables, so the
      ...
      const error = await runEffect(buildAnnotation({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage build annotations",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the buildAnnotation mutation wrapper
