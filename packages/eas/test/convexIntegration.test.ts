...
import { convexIntegration } from "../src/operations/convexIntegration.ts";
...
// without the privilege to invoke convexIntegration queries and emits
...
describe("convexIntegration", () => {
  it(
    "happy path - executes the convexIntegration query and returns a clientIdentifier",
    async () => {
      const result = await runEffect(convexIntegration({}));
  ...
  it(
    ...
    async () => {
      // The convexIntegration query wrapper has no input variables, so the
      ...
      const error = await runEffect(convexIntegration({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to read the Convex integration",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the convexIntegration query wrapper is
