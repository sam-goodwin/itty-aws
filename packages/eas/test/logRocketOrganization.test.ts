...
import { logRocketOrganization } from "../src/operations/logRocketOrganization.ts";
...
describe("logRocketOrganization", () => {
  it("happy path - executes the logRocketOrganization mutation and returns a result", async () => {
    const result = await runEffect(logRocketOrganization({}));
    expect(result).toBeDefined();
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      logRocketOrganization({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(logRocketOrganization({}).pipe(Effect.flip));
