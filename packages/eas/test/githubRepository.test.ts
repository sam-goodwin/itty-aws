...
import { githubRepository } from "../src/operations/githubRepository.ts";
...
// without the privilege to invoke githubRepository mutations and emits
...
describe("githubRepository", () => {
  it(
    "happy path - executes the githubRepository mutation and returns a result",
    async () => {
      const result = await runEffect(githubRepository({}));
  ...
  it(
    ...
    async () => {
      // The githubRepository mutation wrapper has no input variables, so
      ...
      const error = await runEffect(githubRepository({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage GitHub repositories",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the githubRepository mutation wrapper
