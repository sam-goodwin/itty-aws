...
import { iosAppBuildCredentials } from "../src/operations/iosAppBuildCredentials.ts";
...
// without the privilege to invoke iosAppBuildCredentials mutations and emits
...
describe("iosAppBuildCredentials", () => {
  it(
    "happy path - executes the iosAppBuildCredentials mutation and returns a result",
    async () => {
      const result = await runEffect(iosAppBuildCredentials({}));
  ...
  it(
    ...
    async () => {
      // The iosAppBuildCredentials mutation wrapper has no input variables,
      ...
      const error = await runEffect(
        iosAppBuildCredentials({}).pipe(Effect.flip),
      );
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage iOS app build credentials",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the iosAppBuildCredentials mutation
