...
import { androidFcm } from "../src/operations/androidFcm.ts";
...
// without the privilege to invoke FCM credential mutations and emits
...
describe("androidFcm", () => {
  it(
    "happy path - executes the androidFcm mutation and returns a result",
    async () => {
      const result = await runEffect(androidFcm({}));
  ...
  it(
    ...
    async () => {
      // The androidFcm mutation wrapper has no input variables, so the only
      ...
      const error = await runEffect(androidFcm({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage FCM credentials",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the androidFcm mutation wrapper is
