...
import { experimentation } from "../src/operations/experimentation.ts";
...
// without the privilege to invoke experimentation queries and emits
...
describe("experimentation", () => {
  it(
    "happy path - executes the experimentation query and returns the experimentation configuration",
    async () => {
      const result = await runEffect(experimentation({}));
      expect(result).toBeDefined();
      expect(typeof result.deviceExperimentationUnit).toBe("string");
    },
    { timeout: 30_000 },
  );
  ...
  it(
    ...
    async () => {
      // The experimentation query wrapper has no input variables, so the
      ...
      const error = await runEffect(experimentation({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to read experimentation configuration",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the experimentation query wrapper is
