...
import { appleDistributionCertificate } from "../src/operations/appleDistributionCertificate.ts";
...
// without the privilege to invoke distribution certificate mutations and emits
...
describe("appleDistributionCertificate", () => {
  it(
    "happy path - executes the appleDistributionCertificate mutation and returns a result",
    async () => {
      const result = await runEffect(appleDistributionCertificate({}));
  ...
  it(
    ...
    async () => {
      const error = await Effect.runPromise(
        appleDistributionCertificate({}).pipe(
          Effect.flip,
  ...
  it(
    ...
    async () => {
      // The appleDistributionCertificate mutation wrapper has no input
      ...
      const error = await runEffect(
        appleDistributionCertificate({}).pipe(Effect.flip),
      );
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage distribution certificates",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the appleDistributionCertificate
      // mutation wrapper is rejected with extensions.errorCode =
      // UNAUTHORIZED_ERROR — distinct from a missing/invalid token
