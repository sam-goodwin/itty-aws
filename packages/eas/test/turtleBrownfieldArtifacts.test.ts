...
import { turtleBrownfieldArtifacts } from "../src/operations/turtleBrownfieldArtifacts.ts";
...
describe("turtleBrownfieldArtifacts", () => {
  it("happy path - executes the turtleBrownfieldArtifacts mutation and returns a result", async () => {
    const result = await runEffect(turtleBrownfieldArtifacts({}));
    expect(result).toBeDefined();
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      turtleBrownfieldArtifacts({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(turtleBrownfieldArtifacts({}).pipe(Effect.flip));
