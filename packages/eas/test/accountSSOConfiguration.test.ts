...
import { accountSSOConfiguration } from "../src/operations/accountSSOConfiguration.ts";
...
// without the privilege to invoke SSO configuration mutations and emits
...
describe("accountSSOConfiguration", () => {
  it(
    "happy path - executes the accountSSOConfiguration mutation and returns a result",
    async () => {
      const result = await runEffect(accountSSOConfiguration({}));
  ...
  it(
    ...
    async () => {
      // The accountSSOConfiguration mutation wrapper has no input variables,
      ...
      const error = await runEffect(
        accountSSOConfiguration({}).pipe(Effect.flip),
      );
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage SSO configurations",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // SSO-admin role required to invoke this mutation wrapper is rejected
