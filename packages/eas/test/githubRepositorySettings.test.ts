...
import { githubRepositorySettings } from "../src/operations/githubRepositorySettings.ts";
...
// without the privilege to invoke githubRepositorySettings mutations and
// emits extensions.errorCode = UNAUTHORIZED_ERROR (→ EasUnauthorizedOperation).
...
describe("githubRepositorySettings", () => {
  it(
    "happy path - executes the githubRepositorySettings mutation and returns a result",
    async () => {
      const result = await runEffect(githubRepositorySettings({}));
  ...
  it(
    ...
    async () => {
      // The githubRepositorySettings mutation wrapper has no input
      // variables, so the only server-side rejection path is GraphQL
      ...
      const error = await runEffect(
        githubRepositorySettings({}).pipe(Effect.flip),
      );
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage GitHub repository settings",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the githubRepositorySettings mutation
