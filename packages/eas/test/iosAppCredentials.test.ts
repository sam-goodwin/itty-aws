...
import { iosAppCredentials } from "../src/operations/iosAppCredentials.ts";
...
describe("iosAppCredentials", () => {
  it("happy path - executes the iosAppCredentials mutation and returns a result", async () => {
    const result = await runEffect(iosAppCredentials({}));
    expect(result).toBeDefined();
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      iosAppCredentials({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(iosAppCredentials({}).pipe(Effect.flip));
