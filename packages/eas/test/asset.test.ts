...
import { asset } from "../src/operations/asset.ts";
...
// without the privilege to invoke asset mutations and emits
...
describe("asset", () => {
  it(
    "happy path - executes the asset mutation and returns a result",
    async () => {
      const result = await runEffect(asset({}));
  ...
  it(
    ...
    async () => {
      // The asset mutation wrapper has no input variables, so the only
      ...
      const error = await runEffect(asset({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage assets",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the asset mutation wrapper is rejected
