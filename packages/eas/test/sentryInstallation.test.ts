...
import { sentryInstallation } from "../src/operations/sentryInstallation.ts";
...
describe("sentryInstallation", () => {
  it("happy path - executes the sentryInstallation mutation and returns a result", async () => {
    const result = await runEffect(sentryInstallation({}));
    expect(result).toBeDefined();
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      sentryInstallation({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(sentryInstallation({}).pipe(Effect.flip));
