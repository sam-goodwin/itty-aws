...
import { echoRepository } from "../src/operations/echoRepository.ts";
...
// without the privilege to invoke echoRepository mutations and emits
...
describe("echoRepository", () => {
  it(
    "happy path - executes the echoRepository mutation and returns a result",
    async () => {
      const result = await runEffect(echoRepository({}));
  ...
  it(
    ...
    async () => {
      // The echoRepository mutation wrapper has no input variables, so the
      ...
      const error = await runEffect(echoRepository({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage Echo repositories",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the echoRepository mutation wrapper
