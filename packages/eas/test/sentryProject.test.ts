...
import { sentryProject } from "../src/operations/sentryProject.ts";
...
describe("sentryProject", () => {
  it("happy path - executes the sentryProject mutation and returns a result", async () => {
    const result = await runEffect(sentryProject({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(sentryProject({}).pipe(Effect.flip));
