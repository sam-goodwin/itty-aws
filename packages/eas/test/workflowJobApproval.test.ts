...
import { workflowJobApproval } from "../src/operations/workflowJobApproval.ts";
...
describe("workflowJobApproval", () => {
  it("happy path - executes the workflowJobApproval mutation and returns a result", async () => {
    const result = await runEffect(workflowJobApproval({}));
    expect(result).toBeDefined();
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      workflowJobApproval({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(workflowJobApproval({}).pipe(Effect.flip));
