...
import { websiteNotifications } from "../src/operations/websiteNotifications.ts";
...
describe("websiteNotifications", () => {
  it("happy path - executes the websiteNotifications mutation and returns updateAllWebsiteNotificationReadStateAsRead", async () => {
    const result = await runEffect(websiteNotifications({}));
    expect(result).toBeDefined();
    expect(typeof result.updateAllWebsiteNotificationReadStateAsRead).toBe("boolean");
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      websiteNotifications({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(websiteNotifications({}).pipe(Effect.flip));
