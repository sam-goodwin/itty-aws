...
import { echoChat } from "../src/operations/echoChat.ts";
...
// without the privilege to invoke echoChat mutations and emits
...
describe("echoChat", () => {
  it(
    "happy path - executes the echoChat mutation and returns a result",
    async () => {
      const result = await runEffect(echoChat({}));
  ...
  it(
    ...
    async () => {
      // The echoChat mutation wrapper has no input variables, so the only
      ...
      const error = await runEffect(echoChat({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage Echo chats",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the echoChat mutation wrapper is
