...
// without the privilege to invoke deployments mutations and emits
...
describe("deployments", () => {
  it(
    "happy path - executes the deployments mutation and returns a result",
    async () => {
      const result = await runEffect(deployments({}));
  ...
  it(
    ...
    async () => {
      // The deployments mutation wrapper has no input variables, so the
      ...
      const error = await runEffect(deployments({}).pipe(Effect.flip));
  ...
  it(
    ...
    async () => {
      // Deployment mutations require the target app to already have a dev
      // domain name configured. When that prerequisite is missing the EAS
      // backend emits extensions.errorCode = APP_NO_DEV_DOMAIN_NAME.
      const error = await runEffect(deployments({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage deployments",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the deployments mutation wrapper is
