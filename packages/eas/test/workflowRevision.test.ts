...
import { workflowRevision } from "../src/operations/workflowRevision.ts";
...
describe("workflowRevision", () => {
  it("happy path - executes the workflowRevision mutation and returns a result", async () => {
    const result = await runEffect(workflowRevision({}));
    expect(result).toBeDefined();
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      workflowRevision({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(workflowRevision({}).pipe(Effect.flip));
