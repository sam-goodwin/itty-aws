...
import { githubAppInstallation } from "../src/operations/githubAppInstallation.ts";
...
// without the privilege to invoke githubAppInstallation mutations and emits
...
describe("githubAppInstallation", () => {
  it(
    "happy path - executes the githubAppInstallation mutation and returns a result",
    async () => {
      const result = await runEffect(githubAppInstallation({}));
  ...
  it(
    ...
    async () => {
      // The githubAppInstallation mutation wrapper has no input variables,
      ...
      const error = await runEffect(
        githubAppInstallation({}).pipe(Effect.flip),
      );
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage GitHub App installations",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the githubAppInstallation mutation
