...
import { notificationPreference } from "../src/operations/notificationPreference.ts";
...
describe("notificationPreference", () => {
  it("happy path - executes the notificationPreference mutation and returns a result", async () => {
    const result = await runEffect(notificationPreference({}));
    expect(result).toBeDefined();
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      notificationPreference({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(notificationPreference({}).pipe(Effect.flip));
