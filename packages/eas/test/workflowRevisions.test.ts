...
import { workflowRevisions } from "../src/operations/workflowRevisions.ts";
...
describe("workflowRevisions", () => {
  it("happy path - executes the workflowRevisions query and returns a result", async () => {
    const result = await runEffect(workflowRevisions({}));
    expect(result).toBeDefined();
  }, { timeout: 30_000 });

  it("error - Unauthorized when called with an invalid bearer token", async () => {
    const error = await Effect.runPromise(
      workflowRevisions({}).pipe(
        Effect.flip,
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(workflowRevisions({}).pipe(Effect.flip));
