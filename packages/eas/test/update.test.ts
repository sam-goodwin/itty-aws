...
describe("update", () => {
  it("happy path - executes the update mutation and returns a result", async () => {
    const result = await runEffect(update({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(update({}).pipe(Effect.flip));
