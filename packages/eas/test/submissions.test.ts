...
import { submissions } from "../src/operations/submissions.ts";
...
describe("submissions", () => {
  it("happy path - executes the submissions query and returns a result", async () => {
    const result = await runEffect(submissions({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(submissions({}).pipe(Effect.flip));
