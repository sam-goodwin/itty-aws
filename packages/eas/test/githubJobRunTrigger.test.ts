...
import { githubJobRunTrigger } from "../src/operations/githubJobRunTrigger.ts";
...
// without the privilege to invoke githubJobRunTrigger mutations and emits
...
describe("githubJobRunTrigger", () => {
  it(
    "happy path - executes the githubJobRunTrigger mutation and returns a result",
    async () => {
      const result = await runEffect(githubJobRunTrigger({}));
  ...
  it(
    ...
    async () => {
      // The githubJobRunTrigger mutation wrapper has no input variables, so
      ...
      const error = await runEffect(githubJobRunTrigger({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage GitHub job run triggers",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the githubJobRunTrigger mutation
