...
import { robot } from "../src/operations/robot.ts";
...
describe("robot", () => {
  it("happy path - executes the robot mutation and returns a result", async () => {
    const result = await runEffect(robot({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(robot({}).pipe(Effect.flip));
