...
import { appStoreConnectApp } from "../src/operations/appStoreConnectApp.ts";
...
// without the privilege to invoke App Store Connect app mutations and emits
...
describe("appStoreConnectApp", () => {
  it(
    "happy path - executes the appStoreConnectApp mutation and returns a result",
    async () => {
      const result = await runEffect(appStoreConnectApp({}));
  ...
  it(
    ...
    async () => {
      // The appStoreConnectApp mutation wrapper has no input variables, so
      ...
      const error = await runEffect(appStoreConnectApp({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage App Store Connect apps",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the appStoreConnectApp mutation
