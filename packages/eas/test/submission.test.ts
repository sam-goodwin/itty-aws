...
import { submission } from "../src/operations/submission.ts";
...
describe("submission", () => {
  it("happy path - executes the submission mutation and returns a result", async () => {
    const result = await runEffect(submission({}));
  ...
  it("error - EasValidationError when the GraphQL operation is rejected by the server", async () => {
    const error = await runEffect(submission({}).pipe(Effect.flip));
