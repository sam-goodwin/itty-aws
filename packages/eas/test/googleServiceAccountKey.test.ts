...
import { googleServiceAccountKey } from "../src/operations/googleServiceAccountKey.ts";
...
// without the privilege to invoke googleServiceAccountKey mutations and
...
describe("googleServiceAccountKey", () => {
  it(
    "happy path - executes the googleServiceAccountKey mutation and returns a result",
    async () => {
      const result = await runEffect(googleServiceAccountKey({}));
  ...
  it(
    ...
    async () => {
      // The googleServiceAccountKey mutation wrapper has no input
      ...
      const error = await runEffect(
        googleServiceAccountKey({}).pipe(Effect.flip),
      );
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage Google Service Account Keys",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the googleServiceAccountKey mutation
