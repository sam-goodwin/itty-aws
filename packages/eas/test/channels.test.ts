...
import { channels } from "../src/operations/channels.ts";
...
// without the privilege to invoke channels queries and emits
...
describe("channels", () => {
  it(
    "happy path - executes the channels query and returns a result",
    async () => {
      const result = await runEffect(channels({}));
  ...
  it(
    ...
    async () => {
      // The channels query wrapper has no input variables, so the only
      ...
      const error = await runEffect(channels({}).pipe(Effect.flip));
  ...
  it(
    "error - EasChannelAlreadyExists when a channel with the same name already exists",
    async () => {
      // The EAS backend rejects channel mutations whose target name collides
      // with an existing channel by emitting extensions.errorCode =
  ...
  it(
    ...
    async () => {
      // The channels query is implicitly scoped by the calling actor's
      // current experience. When that resolution fails (the experience has
      // been deleted, the actor is no longer associated with it, etc.), the
      // EAS backend emits extensions.errorCode = EXPERIENCE_NOT_FOUND.
      const error = await runEffect(channels({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to list channels",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the channels query wrapper is rejected
