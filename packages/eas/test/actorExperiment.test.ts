...
import { actorExperiment } from "../src/operations/actorExperiment.ts";
...
// without the privilege to invoke actor experiment mutations and emits
...
describe("actorExperiment", () => {
  it(
    "happy path - executes the actorExperiment mutation and returns a result",
    async () => {
      const result = await runEffect(actorExperiment({}));
  ...
  it(
    ...
    async () => {
      // The actorExperiment mutation wrapper has no input variables, so the
      ...
      const error = await runEffect(actorExperiment({}).pipe(Effect.flip));
  ...
  it(
    "error - EasUnauthorizedOperation when the actor lacks permission to manage experiments",
    async () => {
      // A token that authenticates as a real actor but does not carry the
      // role/grant required to invoke the actorExperiment mutation wrapper
