#!/usr/bin/env bun
/**
 * generate — turn the Smithy JSON model in .generated-specs into the Coolify
 * Effect SDK.
 *
 * Input:  .generated-specs/coolify.json  (written by scripts/convert.ts)
 * Output: src/services/coolify.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is Coolify's provider spec.
 * Coolify keeps the wire's snake_case member names on the TS surface, so no
...
/** Coolify's provider spec for the shared smithy→SDK compiler. */
const coolifySpec: SdkSpec = {
  ...
  // names for Coolify), so no runtime case discrimination is needed.
  union: ({ name, caseTargets, tsRef }) => [
    `export type ${name} = ${caseTargets.map(tsRef).join(" | ") || "unknown"};`,
    `export const ${name} = /*@__PURE__*/ S.Unknown as any as S.Schema<${name}>;\n`,
  ],

  // One pagination profile: cursor mode (inputToken `cursor`,
  ...
  operationDecl: {
    contextType: "CoolifyOpContext",
    commonErrorType: "CoolifyOpError",
    commonErrorClasses: ["UnknownCoolifyError"],
    protocol: "CoolifyProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/distilled-spec-coolify)",
...
runGeneratorCli({
  description: "Generate the Coolify Effect SDK from the Smithy model",
