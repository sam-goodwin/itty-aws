...
import { keysRemoveRoles } from "../src/operations/keysRemoveRoles.ts";
...
describe("keysRemoveRoles", () => {
  it(
    "happy path - removes roles from a key",
    { timeout: 30_000 },
    async () => {
      const apiId = `api_${testRunId}`;
      const apiName = `distilled-unkey-api-remove-roles-${testRunId}`;
      const keyId = `key_${testRunId}`;
      const keyName = `distilled-unkey-key-remove-roles-${testRunId}`;
      const roles = [
        `distilled-unkey-role-reader-${testRunId}`,
        `distilled-unkey-role-writer-${testRunId}`,
      ];
      const roleData = roles.map((name, index) => ({
        id: `role_${testRunId}_${index}`,
        name,
        description: `Role ${index} for ${testRunId}`,
        permissions: [
          {
            id: `perm_${testRunId}_${index}`,
            name: `distilled-unkey-permission-${index}-${testRunId}`,
            slug: `distilled.unkey.${testRunId}.${index}`,
            description: `Permission ${index} for ${testRunId}`,
          },
        ],
      }));
      const keyRoles = new Map<string, Array<string>>();
      let apiExists = false;
      let deletedApiId = "";
      let requestMethod = "";
      let requestUrl = "";
      ...
      await withServer(
        (req, res) => {
          void readBody(req).then((body) => {
            ...
            if (req.url === "/v2/keys.createKey") {
              ...
              keyRoles.set(keyId, parsed.roles ?? []);
              sendJson(res, 200, {
            ...
            if (req.url === "/v2/keys.removeRoles") {
              ...
              const parsed = JSON.parse(body);
              const currentRoles = keyRoles.get(parsed.keyId);
              ...
              keyRoles.set(
                parsed.keyId,
                currentRoles.filter((role) => !parsed.roles.includes(role)),
              );

              sendJson(res, 200, {
                meta: { requestId: `req_remove_${testRunId}` },
                data: roleData,
              });
              return;
            }
        ...
        async (baseUrl) => {
          ...
          const effect = Effect.gen(function* () {
            ...
            const result = yield* keysRemoveRoles({
              keyId: createdKey.data.keyId,
              roles,
            });

            expect(requestMethod).toBe("POST");
            expect(requestUrl).toBe("/v2/keys.removeRoles");
            expect(authorization).toBe("Bearer unkey_test");
            expect(JSON.parse(requestBody)).toEqual({ keyId, roles });
            expect(result.meta.requestId).toBe(`req_remove_${testRunId}`);
            expect(result.data).toEqual(roleData);
            expect(keyRoles.get(keyId)).toEqual([]);
          }).pipe(
            Effect.ensuring(
              Effect.gen(function* () {
                if (createdApiId !== "") {
  ...
  it(
    "error - BadRequest when roles are empty",
    { timeout: 30_000 },
    async () => {
      await withServer(
        (_req, res) => {
        ...
        async (baseUrl) => {
          const error = await runWithBaseUrl(
            ...
            keysRemoveRoles({
              ...
              roles: [`distilled-unkey-role-forbidden-${testRunId}`],
            }).pipe(Effect.flip),
          );
        ...
        async (baseUrl) => {
          const error = await runWithBaseUrl(
            ...
            keysRemoveRoles({
              ...
              roles: [`distilled-unkey-role-missing-${testRunId}`],
            }).pipe(Effect.flip),
          );
