...
describe("ratelimitDeleteOverride", () => {
  it(
    "happy path - deletes a ratelimit override",
    { timeout: 30_000 },
    async () => {
      ...
      await withServer(
        (req, res) => {
          void readBody(req).then((body) => {
            ...
            if (req.url === "/v2/ratelimit.deleteOverride") {
              ...
              const parsed = JSON.parse(body);
              const key = overrideKey(parsed.namespace, parsed.identifier);
        ...
        async (baseUrl) => {
          ...
          const effect = Effect.gen(function* () {
            ...
            const result = yield* ratelimitDeleteOverride({
              namespace: overrideInput.namespace,
              identifier: overrideInput.identifier,
            });
            ...
            expect(requestUrl).toBe("/v2/ratelimit.deleteOverride");
            ...
            expect(
              overrides.has(
                overrideKey(overrideInput.namespace, overrideInput.identifier),
              ),
            ).toBe(false);
          }).pipe(
            Effect.ensuring(
  ...
  it(
    "error - Forbidden when credentials lack override delete access",
    { timeout: 30_000 },
    async () => {
      await withServer(
        (_req, res) => {
          sendApiError(
            ...
            403,
            "Forbidden",
            "The root key lacks ratelimit override delete access.",
