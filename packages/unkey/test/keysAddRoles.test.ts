...
import { keysAddRoles } from "../src/operations/keysAddRoles.ts";
...
describe("keysAddRoles", () => {
  it(
    "happy path - adds roles to a key",
    { timeout: 30_000 },
    async () => {
      const apiId = `api_${testRunId}`;
      const apiName = `distilled-unkey-api-add-roles-${testRunId}`;
      const keyId = `key_${testRunId}`;
      const keyName = `distilled-unkey-key-add-roles-${testRunId}`;
      const roles = [
      ...
      await withServer(
        ...
        async (baseUrl) => {
          ...
          const effect = Effect.gen(function* () {
            ...
            const result = yield* keysAddRoles({
              keyId: createdKey.data.keyId,
              roles,
            });

            expect(requestMethod).toBe("POST");
            expect(requestUrl).toBe("/v2/keys.addRoles");
