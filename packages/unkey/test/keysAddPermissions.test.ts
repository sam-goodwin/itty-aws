...
import { keysAddPermissions } from "../src/operations/keysAddPermissions.ts";
...
describe("keysAddPermissions", () => {
  it(
    "happy path - adds permissions to a key",
    { timeout: 30_000 },
    async () => {
      const apiId = `api_${testRunId}`;
      const apiName = `distilled-unkey-api-add-permissions-${testRunId}`;
      const keyId = `key_${testRunId}`;
      const keyName = `distilled-unkey-key-add-permissions-${testRunId}`;
      const permissions = [
      ...
      await withServer(
        (req, res) => {
          void readBody(req).then((body) => {
            ...
            if (req.url === "/v2/keys.addPermissions") {
              ...
              sendJson(res, 200, {
                meta: { requestId: `req_add_${testRunId}` },
                data: permissionData,
              });
              return;
            }
        ...
        async (baseUrl) => {
          ...
          const effect = Effect.gen(function* () {
            ...
            const result = yield* keysAddPermissions({
              keyId: createdKey.data.keyId,
              permissions,
            });

            expect(requestMethod).toBe("POST");
            expect(requestUrl).toBe("/v2/keys.addPermissions");
            expect(authorization).toBe("Bearer unkey_test");
            expect(JSON.parse(requestBody)).toEqual({ keyId, permissions });
            expect(result.meta.requestId).toBe(`req_add_${testRunId}`);
