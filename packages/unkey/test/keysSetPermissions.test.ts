...
import { keysSetPermissions } from "../src/operations/keysSetPermissions.ts";
...
describe("keysSetPermissions", () => {
  it(
    "happy path - sets permissions on a key",
    { timeout: 30_000 },
    async () => {
      const apiId = `api_${testRunId}`;
      const apiName = `distilled-unkey-api-set-permissions-${testRunId}`;
      const keyId = `key_${testRunId}`;
      const keyName = `distilled-unkey-key-set-permissions-${testRunId}`;
      const initialPermissions = [`distilled.unkey.${testRunId}.old`];
      const permissions = [
      ...
      await withServer(
        (req, res) => {
          void readBody(req).then((body) => {
            ...
            if (req.url === "/v2/keys.setPermissions") {
              ...
              const parsed = JSON.parse(body);
              if (!keyPermissions.has(parsed.keyId)) {
                sendApiError(
                  res,
                  404,
                  "Not Found",
              ...
              keyPermissions.set(parsed.keyId, parsed.permissions);
              sendJson(res, 200, {
        ...
        async (baseUrl) => {
          ...
          const effect = Effect.gen(function* () {
            ...
            const result = yield* keysSetPermissions({
              keyId: createdKey.data.keyId,
              permissions,
            });

            expect(requestMethod).toBe("POST");
            expect(requestUrl).toBe("/v2/keys.setPermissions");
            ...
            expect(keyPermissions.get(keyId)).toEqual(permissions);
          }).pipe(
            Effect.ensuring(
              Effect.gen(function* () {
                if (createdApiId !== "") {
