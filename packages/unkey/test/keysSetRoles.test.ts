...
import { keysSetRoles } from "../src/operations/keysSetRoles.ts";
...
describe("keysSetRoles", () => {
  it(
    "happy path - sets roles on a key",
    { timeout: 30_000 },
    async () => {
      const apiId = `api_${testRunId}`;
      const apiName = `distilled-unkey-api-set-roles-${testRunId}`;
      const keyId = `key_${testRunId}`;
      const keyName = `distilled-unkey-key-set-roles-${testRunId}`;
      const initialRoles = [`distilled-unkey-role-old-${testRunId}`];
      const roles = [
      ...
      await withServer(
        (req, res) => {
          void readBody(req).then((body) => {
            ...
            if (req.url === "/v2/keys.setRoles") {
              ...
              keyRoles.set(parsed.keyId, parsed.roles);
              sendJson(res, 200, {
        ...
        async (baseUrl) => {
          ...
          const effect = Effect.gen(function* () {
            ...
            const result = yield* keysSetRoles({
              keyId: createdKey.data.keyId,
              roles,
            });

            expect(requestMethod).toBe("POST");
            expect(requestUrl).toBe("/v2/keys.setRoles");
            ...
            expect(keyRoles.get(keyId)).toEqual(roles);
          }).pipe(
            Effect.ensuring(
              Effect.gen(function* () {
                if (createdApiId !== "") {
