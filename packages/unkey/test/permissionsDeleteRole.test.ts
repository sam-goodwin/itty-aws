...
describe("permissionsDeleteRole", () => {
  it(
    "happy path - deletes a role",
    { timeout: 30_000 },
    async () => {
      const roleId = `role_${testRunId}`;
      const roleInput = {
        name: `distilled-unkey-role-delete-${testRunId}`,
        description: `Role deleted by distilled test ${testRunId}`,
      };
      ...
      await withServer(
        ...
        async (baseUrl) => {
          ...
          const effect = Effect.gen(function* () {
            ...
            const result = yield* permissionsDeleteRole({
              role: createdRole,
            });
            createdRole = "";

            expect(requestMethod).toBe("POST");
            expect(requestUrl).toBe("/v2/permissions.deleteRole");
            ...
            expect(roles.has(roleInput.name)).toBe(false);
          }).pipe(
            Effect.ensuring(
              Effect.gen(function* () {
                if (createdRole !== "") {
      ...
      await withServer(
        ...
        async (baseUrl) => {
          const error = await runWithBaseUrl(
            baseUrl,
            permissionsDeleteRole({ role: "" }).pipe(Effect.flip),
          );
  ...
  it(
    "error - Forbidden when credentials lack role delete access",
    { timeout: 30_000 },
    async () => {
      await withServer(
        (_req, res) => {
          sendApiError(
            ...
            403,
            "Forbidden",
            "The root key lacks role delete access.",
