...
import { keysUpdateCredits } from "../src/operations/keysUpdateCredits.ts";
...
describe("keysUpdateCredits", () => {
  it(
    "happy path - updates key credits",
    { timeout: 30_000 },
    async () => {
      const apiId = `api_${testRunId}`;
      const apiName = `distilled-unkey-api-update-credits-${testRunId}`;
      const keyId = `key_${testRunId}`;
      const keyName = `distilled-unkey-key-update-credits-${testRunId}`;
      ...
      const updatedCredits = {
        remaining: 750,
        refill: initialCredits.refill,
      };
      const keyCredits = new Map<string, typeof updatedCredits>();
      let apiExists = false;
      let deletedApiId = "";
      let requestMethod = "";
      let requestUrl = "";
      ...
      await withServer(
        (req, res) => {
          void readBody(req).then((body) => {
            ...
            if (req.url === "/v2/keys.createKey") {
              ...
              keyCredits.set(keyId, parsed.credits);
              sendJson(res, 200, {
            ...
            if (req.url === "/v2/keys.updateCredits") {
              ...
              const parsed = JSON.parse(body);
              const currentCredits = keyCredits.get(parsed.keyId);
              ...
              keyCredits.set(parsed.keyId, updatedCredits);
              sendJson(res, 200, {
        ...
        async (baseUrl) => {
          ...
          const effect = Effect.gen(function* () {
            ...
            const result = yield* keysUpdateCredits({
            ...
            expect(requestUrl).toBe("/v2/keys.updateCredits");
            ...
            expect(result.data).toEqual(updatedCredits);
            expect(keyCredits.get(keyId)).toEqual(updatedCredits);
          }).pipe(
            Effect.ensuring(
              Effect.gen(function* () {
                if (createdApiId !== "") {
  ...
  it(
    "error - BadRequest when the value is invalid",
    { timeout: 30_000 },
    async () => {
      await withServer(
        (_req, res) => {
