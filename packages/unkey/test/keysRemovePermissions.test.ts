...
import { keysRemovePermissions } from "../src/operations/keysRemovePermissions.ts";
...
describe("keysRemovePermissions", () => {
  it(
    "happy path - removes permissions from a key",
    { timeout: 30_000 },
    async () => {
      const apiId = `api_${testRunId}`;
      const apiName = `distilled-unkey-api-remove-permissions-${testRunId}`;
      const keyId = `key_${testRunId}`;
      const keyName = `distilled-unkey-key-remove-permissions-${testRunId}`;
      const permissions = [
        `distilled.unkey.${testRunId}.read`,
        `distilled.unkey.${testRunId}.write`,
      ];
      const permissionData = permissions.map((slug, index) => ({
        id: `perm_${testRunId}_${index}`,
        name: `distilled-unkey-permission-${index}-${testRunId}`,
        slug,
        description: `Permission ${index} for ${testRunId}`,
      }));
      const keyPermissions = new Map<string, Array<string>>();
      let apiExists = false;
      let deletedApiId = "";
      let requestMethod = "";
      let requestUrl = "";
      ...
      await withServer(
        (req, res) => {
          void readBody(req).then((body) => {
            ...
            if (req.url === "/v2/keys.createKey") {
              ...
              keyPermissions.set(keyId, parsed.permissions ?? []);
              sendJson(res, 200, {
            ...
            if (req.url === "/v2/keys.removePermissions") {
              ...
              const parsed = JSON.parse(body);
              const currentPermissions = keyPermissions.get(parsed.keyId);
              ...
              keyPermissions.set(
                ...
                currentPermissions.filter(
                  (permission) => !parsed.permissions.includes(permission),
                ),
              );
        ...
        async (baseUrl) => {
          ...
          const effect = Effect.gen(function* () {
            ...
            const result = yield* keysRemovePermissions({
              keyId: createdKey.data.keyId,
              permissions,
            });

            expect(requestMethod).toBe("POST");
            expect(requestUrl).toBe("/v2/keys.removePermissions");
            expect(authorization).toBe("Bearer unkey_test");
            expect(JSON.parse(requestBody)).toEqual({ keyId, permissions });
            expect(result.meta.requestId).toBe(`req_remove_${testRunId}`);
            expect(result.data).toEqual(permissionData);
            expect(keyPermissions.get(keyId)).toEqual([]);
          }).pipe(
            Effect.ensuring(
              Effect.gen(function* () {
                if (createdApiId !== "") {
  ...
  it(
    "error - BadRequest when permissions are empty",
    { timeout: 30_000 },
    async () => {
      await withServer(
        (_req, res) => {
