...
describe("permissionsCreatePermission", () => {
  it(
    "happy path - creates a permission",
    { timeout: 30_000 },
    async () => {
      const permissionId = `perm_${testRunId}`;
      const permissionInput = {
        name: `distilled-unkey-permission-create-${testRunId}`,
        slug: `distilled.unkey.${testRunId}.create`,
        description: `Permission created by distilled test ${testRunId}`,
      };
      ...
      await withServer(
        ...
        async (baseUrl) => {
          ...
          const effect = Effect.gen(function* () {
            const result = yield* permissionsCreatePermission(permissionInput);
            createdPermission = permissionInput.slug;

            expect(requestMethod).toBe("POST");
            expect(requestUrl).toBe("/v2/permissions.createPermission");
            expect(authorization).toBe("Bearer unkey_test");
            expect(JSON.parse(requestBody)).toEqual(permissionInput);
            expect(result.meta.requestId).toBe(`req_create_${testRunId}`);
            expect(result.data.permissionId).toBe(permissionId);
            expect(permissions.get(permissionInput.slug)).toBe(permissionId);
          }).pipe(
            Effect.ensuring(
              Effect.gen(function* () {
  ...
  it(
    "error - BadRequest when the slug is empty",
    { timeout: 30_000 },
    async () => {
      await withServer(
        (_req, res) => {
          sendApiError(
            ...
            400,
            "Bad Request",
            "The permission slug is required.",
        ...
        async (baseUrl) => {
          const error = await runWithBaseUrl(
            ...
            permissionsCreatePermission({
              name: `distilled-unkey-permission-bad-request-${testRunId}`,
              slug: "",
            }).pipe(Effect.flip),
          );
  ...
  it(
    "error - Forbidden when credentials lack permission creation access",
    { timeout: 30_000 },
    async () => {
      await withServer(
        (_req, res) => {
          sendApiError(
            ...
            403,
            "Forbidden",
            "The root key lacks permission creation access.",
        ...
        async (baseUrl) => {
          const error = await runWithBaseUrl(
            baseUrl,
            permissionsCreatePermission({
              name: `distilled-unkey-permission-forbidden-${testRunId}`,
              slug: `distilled.unkey.${testRunId}.forbidden`,
            }).pipe(Effect.flip),
          );
  ...
  it(
    "error - Conflict when the permission already exists",
    { timeout: 30_000 },
    async () => {
      const permissionId = `perm_conflict_${testRunId}`;
      const permissionInput = {
        name: `distilled-unkey-permission-conflict-${testRunId}`,
        slug: `distilled.unkey.${testRunId}.conflict`,
        description: `Duplicate permission test ${testRunId}`,
      };
      const permissions = new Map<string, string>();
      let deletedPermission = "";
      ...
      await withServer(
        (req, res) => {
          void readBody(req).then((body) => {
            if (req.url === "/v2/permissions.createPermission") {
              const parsed = JSON.parse(body);
              if (permissions.has(parsed.slug)) {
                sendApiError(
                  res,
                  409,
                  "Conflict",
                  "A permission with this slug already exists.",
        ...
        async (baseUrl) => {
          ...
          const effect = Effect.gen(function* () {
            ...
            const error =
              yield* permissionsCreatePermission(permissionInput).pipe(
                Effect.flip,
              );
