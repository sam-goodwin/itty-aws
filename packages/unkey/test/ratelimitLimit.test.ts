...
import { ratelimitLimit } from "../src/operations/ratelimitLimit.ts";
...
describe("ratelimitLimit", () => {
  it(
    "happy path - applies a ratelimit check",
    { timeout: 30_000 },
    async () => {
      const input = {
      ...
      await withServer(
        (req, res) => {
          void readBody(req).then((body) => {
            ...
            sendJson(res, 200, {
              meta: { requestId: `req_limit_${testRunId}` },
              data: responseData,
            });
          });
        },
        async (baseUrl) => {
          const result = await runWithBaseUrl(baseUrl, ratelimitLimit(input));

          expect(requestMethod).toBe("POST");
          expect(requestUrl).toBe("/v2/ratelimit.limit");
          expect(authorization).toBe("Bearer unkey_test");
          expect(JSON.parse(requestBody)).toEqual(input);
          expect(result.meta.requestId).toBe(`req_limit_${testRunId}`);
  ...
  it(
    "error - Gone when the namespace has been deleted",
    { timeout: 30_000 },
    async () => {
      await withServer(
        (_req, res) => {
        ...
        async (baseUrl) => {
          const error = await runWithBaseUrl(
            baseUrl,
            ratelimitLimit({
              namespace: `distilled-unkey-ratelimit-deleted-${testRunId}`,
              cost: 1,
              duration: 60_000,
              identifier: `distilled-unkey-user-deleted-${testRunId}`,
              limit: 10,
            }).pipe(Effect.flip),
          );
