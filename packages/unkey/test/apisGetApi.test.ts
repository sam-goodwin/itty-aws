...
describe("apisGetApi", () => {
  it(
    "happy path - retrieves an API namespace",
    { timeout: 30_000 },
    async () => {
      const apiId = `api_${testRunId}`;
      const apiName = `distilled-unkey-api-get-${testRunId}`;
      let apiExists = false;
      let deletedApiId = "";
      let getMethod = "";
      let getUrl = "";
      ...
      await withServer(
        ...
        async (baseUrl) => {
          ...
          const effect = Effect.gen(function* () {
            ...
            const result = yield* apisGetApi({ apiId: createdApiId });

            expect(getMethod).toBe("POST");
            expect(getUrl).toBe("/v2/apis.getApi");
            expect(authorization).toBe("Bearer unkey_test");
            expect(JSON.parse(getBody)).toEqual({ apiId });
            expect(result.meta.requestId).toBe(`req_get_${testRunId}`);
            expect(result.data.id).toBe(apiId);
            expect(result.data.name).toBe(apiName);
          }).pipe(
            Effect.ensuring(
              Effect.gen(function* () {
                if (createdApiId !== "") {
      ...
      await withServer(
        ...
        async (baseUrl) => {
          const error = await runWithBaseUrl(
            baseUrl,
            apisGetApi({ apiId: "" }).pipe(Effect.flip),
          );
  ...
  it(
    "error - Forbidden when credentials lack API read access",
    { timeout: 30_000 },
    async () => {
      await withServer(
        (_req, res) => {
          sendApiError(
            ...
            403,
            "Forbidden",
            "The root key lacks API read access.",
        ...
        async (baseUrl) => {
          const error = await runWithBaseUrl(
            baseUrl,
            apisGetApi({ apiId: `api_forbidden_${testRunId}` }).pipe(
              Effect.flip,
            ),
          );
    ...
    async () => {
      await withServer(
        ...
        async (baseUrl) => {
          const error = await runWithBaseUrl(
            baseUrl,
            apisGetApi({ apiId: `api_missing_${testRunId}` }).pipe(
              Effect.flip,
            ),
          );
