...
import { keysWhoami } from "../src/operations/keysWhoami.ts";
...
describe("keysWhoami", () => {
  it(
    "happy path - gets a key by plaintext",
    { timeout: 30_000 },
    async () => {
      const apiId = `api_${testRunId}`;
      const apiName = `distilled-unkey-api-whoami-${testRunId}`;
      const keyId = `key_${testRunId}`;
      const keyName = `distilled-unkey-key-whoami-${testRunId}`;
      ...
      await withServer(
        (req, res) => {
          void readBody(req).then((body) => {
            ...
            if (req.url === "/v2/keys.whoami") {
              ...
              sendJson(res, 200, {
                meta: { requestId: `req_whoami_${testRunId}` },
                data: key,
              });
              return;
            }
        ...
        async (baseUrl) => {
          ...
          const effect = Effect.gen(function* () {
            ...
            const result = yield* keysWhoami({
              key: createdKey.data.key,
            });

            expect(requestMethod).toBe("POST");
            expect(requestUrl).toBe("/v2/keys.whoami");
            expect(authorization).toBe("Bearer unkey_test");
            expect(JSON.parse(requestBody)).toEqual({
              key: plaintext,
            });
            expect(result.meta.requestId).toBe(`req_whoami_${testRunId}`);
