 ...
 * layer level and have every Intercom API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Intercom from "@distilled.cloud/intercom";
 *
 * myEffect.pipe(Intercom.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Intercom.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Intercom API calls. */
export class Retry extends Context.Service<Retry, Policy>()("IntercomRetry") {}

/** Provides a custom retry policy to every Intercom API call below it. */
