...
import { projectTypeList } from "../src/operations/projectTypeList.ts";
import { runEffect, testRunId } from "./setup.ts";

// Layer that points the SDK at a non-existent path on the real Modrinth
// host. projectTypeList takes no parameters, so the only way to
...
describe("projectTypeList", () => {
  it("returns the public list of project types", async () => {
    // GET /tag/project_type is a public, parameterless read endpoint.
    // Modrinth ships with a baseline set of project types (mod,
    // modpack, resourcepack, shader, etc.) so we expect a non-empty
    // array of strings.
    const projectTypes = await runEffect(projectTypeList({}));
    ...
    for (const projectType of projectTypes) {
      ...
      expect(projectType.length).toBeGreaterThan(0);
    }
  });

  it("returns NotFound when the base URL points to a non-existent path", async () => {
    // projectTypeList has no input parameters, so the only
