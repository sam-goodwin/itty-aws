...
import { deleteNotifications } from "../src/operations/deleteNotifications.ts";
...
describe("deleteNotifications", () => {
  it.skipIf(!OWNED_USER_ID)(
    ...
    async () => {
      // DELETE /notifications?ids=[...] permanently removes every
      // matching notification and returns 204. There is no rollback —
      // the entries are gone — but Modrinth user inboxes hold transient
      // items (project updates, team invites, etc.) that are safe to
      // drop on a test account, and the test is gated on the explicit
      // owned-user opt in. We bootstrap by listing the auth'd user's
      // inbox; if it is empty we still confirm the route accepts an
      // empty array and replies with 204.
      const id = OWNED_USER_ID as string;
      const inbox = await runEffect(getUserNotifications({ id_or_username: id }));

      if (inbox.length === 0) {
        await runEffect(deleteNotifications({ ids: JSON.stringify([]) }));
        return;
      }

      const ids = inbox.slice(0, Math.min(2, inbox.length)).map((n) => n.id);
      // Output schema is Schema.Void; success means no thrown error.
      await runEffect(deleteNotifications({ ids: JSON.stringify(ids) }));

      // After deletion the same ids should no longer be in the inbox.
      const after = await runEffect(getUserNotifications({ id_or_username: id }));
      for (const want of ids) {
        expect(after.some((n) => n.id === want)).toBe(false);
      }
    },
  );
  ...
  it.skipIf(!HAS_API_KEY)(
    ...
    async () => {
      ...
      const error = await runEffect(
        deleteNotifications({ ids: JSON.stringify(ids) }).pipe(Effect.flip),
      );
  ...
  it("returns Unauthorized when no API key is provided", async () => {
    // DELETE /notifications requires auth. Modrinth runs the auth check
    ...
    const error = await Effect.runPromise(
      deleteNotifications({ ids: JSON.stringify([]) }).pipe(
        Effect.flip,
