...
describe("deleteNotification", () => {
  it.skipIf(!OWNED_USER_ID)(
    "deletes an existing notification from the authenticated user",
    async () => {
      // DELETE /notification/{id} permanently removes the notification
      // and returns 204. There is no rollback — the notification is gone
      // — but Modrinth user inboxes hold transient items (project
      // updates, team invites, etc.) that are safe to drop on a test
      // account, and the test is gated on the explicit owned-user opt
      // in. We bootstrap by listing the auth'd user's notifications and
      // deleting the first one; if the inbox is empty we skip the
      // assertion since there is nothing to delete.
      ...
      await runEffect(deleteNotification({ id: notificationId }));

      // After deletion the same id should no longer be in the inbox.
      const after = await runEffect(getUserNotifications({ id_or_username: id }));
      expect(after.some((n) => n.id === notificationId)).toBe(false);
    },
  );
  ...
  it.skipIf(!HAS_API_KEY)(
    ...
    async () => {
      ...
      const error = await runEffect(
        deleteNotification({ id }).pipe(Effect.flip),
      );
  ...
  it("returns Unauthorized when no API key is provided", async () => {
    // DELETE /notification/{id} requires auth. With a base62-shaped id
    // the path validator passes, the auth check fires next, and Modrinth
    // returns 401 without an API key.
    const id = `zz${testRunId.slice(0, 6)}`;
    const error = await Effect.runPromise(
      deleteNotification({ id }).pipe(
        Effect.flip,
