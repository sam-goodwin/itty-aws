 ...
 * layer level and have every Grafana API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Grafana from "@distilled.cloud/grafana";
 *
 * myEffect.pipe(Grafana.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Grafana.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Grafana API calls. */
export class Retry extends Context.Service<Retry, Policy>()("GrafanaRetry") {}

/** Provides a custom retry policy to every Grafana API call below it. */
