#!/usr/bin/env bun
/**
 * generate — turn the Smithy JSON model in .generated-specs into the Grafana
 * Effect SDK.
 *
 * Input:  .generated-specs/grafana.json  (written by scripts/convert.ts)
 * Output: src/services/grafana.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is Grafana's provider spec.
 * Grafana keeps the wire's camelCase / snake_case member names on the TS
...
/** Grafana's provider spec for the shared smithy→SDK compiler. */
const grafanaSpec: SdkSpec = {
  ...
  // names for Grafana), so no runtime case discrimination is needed.
  ...
  sourceNote: ".generated-specs (specs/spec-mirror-grafana)",
...
runGeneratorCli({
  description: "Generate the Grafana Effect SDK from the Smithy model",
