...
// Input Schema
export interface WorkflowCutoverInput {
  organization: string;
  database: string;
  number: number;
}
...
) as unknown as Schema.Codec<WorkflowCutoverInput>;

// Output Schema
export interface WorkflowCutoverOutput {
  id: string;
  name: string;
  number: number;
  state:
...
}) as unknown as Schema.Codec<WorkflowCutoverOutput>;
