...
// GET /guilds/{guild_id}/scheduled-events lists scheduled events for a
...
describe("listGuildScheduledEvents", () => {
  it.skipIf(!TEST_GUILD_ID)(
    "happy path - lists scheduled events for a guild",
    async () => {
      ...
      for (const event of result) {
        ...
        // documented scheduled event shape defensively.
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const e = event as any;
        expect(typeof e.id).toBe("string");
        expect(e.guild_id).toBe(TEST_GUILD_ID!);
        expect(typeof e.name).toBe("string");
        expect(typeof e.scheduled_start_time).toBe("string");
      }
    },
    { timeout: 30_000 },
  );
  ...
  it("error - NotFound for a non-existent guild_id", async () => {
    await runEffect(
      listGuildScheduledEvents({ guild_id: NON_EXISTENT_GUILD_ID }).pipe(
        Effect.flip,
        Effect.map((e) => {
  ...
  it("error - Forbidden or NotFound for a guild the bot is not a member of", async () => {
    await runEffect(
      listGuildScheduledEvents({ guild_id: INACCESSIBLE_GUILD_ID }).pipe(
        Effect.flip,
        Effect.map((e) => {
          // The bot can only list scheduled events in guilds it's a member
