...
import { deleteMyMessageReaction } from "../src/operations/deleteMyMessageReaction.ts";
...
// Requires a text channel where the bot can post and react to its own
// messages.
...
describe("deleteMyMessageReaction", () => {
  it(
    "happy path - adds the bot's own reaction then removes it",
    async () => {
      if (!TEST_CHANNEL_ID) {
        throw new Error(
          "DISCORD_TEST_CHANNEL_ID env var is required for the deleteMyMessageReaction happy path",
        );
      }
      await runEffect(
        Effect.gen(function* () {
          const msg = yield* createMessage({
            ...
            content: `distilled-delete-my-reaction-${testRunId}`,
          });
          return yield* Effect.gen(function* () {
            ...
            const result = yield* deleteMyMessageReaction({
              channel_id: TEST_CHANNEL_ID,
              message_id: msg.id,
              emoji_name: EMOJI,
            });
