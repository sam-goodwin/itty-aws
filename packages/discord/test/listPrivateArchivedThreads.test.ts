...
import { listPrivateArchivedThreads } from "../src/operations/listPrivateArchivedThreads.ts";
...
// GET /channels/{channel_id}/threads/archived/private lists all private
// archived threads in a channel. Requires READ_MESSAGE_HISTORY and
// MANAGE_THREADS. Happy path is gated on DISCORD_TEST_CHANNEL_ID.
const TEST_CHANNEL_ID = process.env.DISCORD_TEST_CHANNEL_ID;

describe("listPrivateArchivedThreads", () => {
  it.skipIf(!TEST_CHANNEL_ID)(
    "happy path - lists private archived threads in a channel",
    async () => {
  ...
  it("error - NotFound for a non-existent channel id", async () => {
    ...
    await runEffect(
      listPrivateArchivedThreads({ channel_id: fakeChannelId }).pipe(
        Effect.flip,
        Effect.map((e) => {
