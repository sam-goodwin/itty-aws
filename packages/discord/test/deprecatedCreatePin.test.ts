...
describe("deprecatedCreatePin", () => {
  it("happy path - pins a freshly created message via the legacy route", async () => {
    if (!TEST_CHANNEL_ID) {
      throw new Error(
        "DISCORD_TEST_CHANNEL_ID env var is required for the deprecatedCreatePin happy path",
      );
    }
    await runEffect(
      Effect.gen(function* () {
        const msg = yield* createMessage({
          ...
          content: `distilled-deprecated-pin-${testRunId}`,
        });
        return yield* deprecatedCreatePin({
          channel_id: TEST_CHANNEL_ID,
          message_id: msg.id,
        }).pipe(
          Effect.tap((result) =>
  ...
  it("error - NotFound for non-existent message_id in a real channel", async () => {
    ...
    // Discord returns 404 NotFound (10008 — message does not exist) when
    // the message_id does not exist in the channel.
    await runEffect(
      deprecatedCreatePin({
        channel_id: TEST_CHANNEL_ID,
