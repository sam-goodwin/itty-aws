...
import { deleteAllMessageReactions } from "../src/operations/deleteAllMessageReactions.ts";
...
describe("deleteAllMessageReactions", () => {
  it("happy path - clears reactions on a freshly posted message", async () => {
    if (!TEST_CHANNEL_ID) {
      throw new Error(
        "DISCORD_TEST_CHANNEL_ID env var is required for the deleteAllMessageReactions happy path",
      );
    }
    void testRunId;
    await runEffect(
      Effect.gen(function* () {
        const msg = yield* createMessage({
          ...
          content: `distilled-clear-reactions-${testRunId}`,
        });
        return yield* deleteAllMessageReactions({
          channel_id: TEST_CHANNEL_ID,
          message_id: msg.id,
        }).pipe(
          Effect.tap(() =>
            // 204 No Content; output schema is Void. Calling against a
            // message with zero reactions is a valid no-op.
            Effect.sync(() => {
