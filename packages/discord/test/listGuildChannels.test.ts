...
// GET /guilds/{guild_id}/channels lists the non-thread channels in a guild.
// The bot must be a member of the guild. The list is allowed to be empty in
// theory, but a real test guild will always have at least one channel.
...
describe("listGuildChannels", () => {
  it.skipIf(!TEST_GUILD_ID)(
    "happy path - lists channels in a guild",
    async () => {
      const result = await runEffect(
        listGuildChannels({ guild_id: TEST_GUILD_ID! }),
      );
      expect(Array.isArray(result)).toBe(true);
      for (const ch of result) {
        // The output schema is Schema.Array(Schema.Unknown); validate the
        // documented channel shape defensively.
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const c = ch as any;
        expect(typeof c.id).toBe("string");
        expect(typeof c.type).toBe("number");
        if (c.guild_id !== undefined) {
          expect(c.guild_id).toBe(TEST_GUILD_ID!);
        }
  ...
  it("error - NotFound for a non-existent guild_id", async () => {
    await runEffect(
      listGuildChannels({ guild_id: NON_EXISTENT_GUILD_ID }).pipe(
        Effect.flip,
        Effect.map((e) => {
  ...
  it("error - Forbidden or NotFound for a guild the bot is not a member of", async () => {
    await runEffect(
      listGuildChannels({ guild_id: INACCESSIBLE_GUILD_ID }).pipe(
        Effect.flip,
        Effect.map((e) => {
          // The bot can only list channels in guilds it's a member of; for
