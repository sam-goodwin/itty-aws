...
import { partnerSdkUnmergeProvisionalAccount } from "../src/operations/partnerSdkUnmergeProvisionalAccount.ts";
...
// POST /partner-sdk/provisional-accounts/unmerge unmerges a provisional
// account from a real Discord account via the Discord Partner SDK flow.
// Requires Discord to have approved the application as a Partner SDK
// integration. The happy path is gated on operator-supplied credentials.
...
describe("partnerSdkUnmergeProvisionalAccount", () => {
  it.skipIf(
    !PARTNER_CLIENT_ID ||
      !PARTNER_EXTERNAL_AUTH_TOKEN ||
      !PARTNER_EXTERNAL_AUTH_TYPE,
  )(
    "happy path - unmerges a provisional account from a Discord account",
    async () => {
      ...
      // The endpoint returns 204 No Content; the typed output is void.
