 ...
 * layer level and have every Clerk API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Clerk from "@distilled.cloud/clerk";
 *
 * myEffect.pipe(Clerk.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Clerk.Retry.Retry, customPolicy));
 * ```
 */
