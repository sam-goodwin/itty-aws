...
import { uploadApplicationLogo as PlatformUploadApplicationLogo } from "../src/operations/platform/applications/uploadApplicationLogo";
...
const appName = (suffix: string): string =>
  `distilled-clerk-uplogo-${suffix}-${testRunId}`;
...
/**
 * Roughly 11 MB string used to provoke PayloadTooLarge. Clerk's logo upload
 * endpoint caps body size at 10 MB.
 */
const OVERSIZED_FILE = "x".repeat(11 * 1024 * 1024);

/**
 * Provision a fresh application for the duration of `testFn`, then delete it
 * afterwards so uploaded logos and the app itself don't accumulate.
 */
...
describe("PlatformUploadApplicationLogo", () => {
  it("uploads a logo to a fresh application", async () => {
    await runEffect(
      withFreshApplication("happy", (applicationID) =>
        Effect.gen(function* () {
          const result = yield* PlatformUploadApplicationLogo({
            applicationID,
            file: TINY_PNG,
          });
  ...
  it("returns BadRequest when the file field is empty", async () => {
    await runEffect(
      withFreshApplication("bad-request", (applicationID) =>
        Effect.gen(function* () {
          const error = yield* PlatformUploadApplicationLogo({
            applicationID,
            file: "",
          }).pipe(Effect.flip);
