/**
 * Meilisearch retry configuration.
 *
 ...
 * layer level and have every Meilisearch API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Meilisearch from "@distilled.cloud/meilisearch";
 *
 * myEffect.pipe(Meilisearch.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Meilisearch.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Meilisearch API calls. */
export class Retry extends Context.Service<Retry, Policy>()(
  "MeilisearchRetry",
) {}

/** Provides a custom retry policy to every Meilisearch API call below it. */
