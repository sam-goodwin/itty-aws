 ...
 * `PutObject`). Inputs are intentionally wide — any standard JS binary
 * source plus Effect `Stream.Stream<Uint8Array>` is accepted. The
 * on-the-wire `Content-Type` is supplied by the caller through the
 * operation's `content-type` header field; the runtime falls back to
 * `application/octet-stream` when none is provided.
 */
