 ...
 * layer level and have every Plaid API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Plaid from "@distilled.cloud/plaid";
 *
 * myEffect.pipe(Plaid.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Plaid.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Plaid API calls. */
export class Retry extends Context.Service<Retry, Policy>()("PlaidRetry") {}

/** Provides a custom retry policy to every Plaid API call below it. */
