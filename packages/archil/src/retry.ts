 ...
 * layer level and have every Archil API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Archil from "@distilled.cloud/archil";
 *
 * myEffect.pipe(Archil.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Archil.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Archil API calls. */
export class Retry extends Context.Service<Retry, Policy>()("ArchilRetry") {}

/** Provides a custom retry policy to every Archil API call below it. */
