 ...
 *
 * Input:  .generated-specs/onepassword.json  (written by scripts/convert.ts)
 * Output: src/services/onepassword.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is 1Password's provider spec.
 * Connect keeps the wire's camelCase member names on the TS surface, so no
...
/** 1Password's provider spec for the shared smithy→SDK compiler. */
const onepasswordSpec: SdkSpec = {
  ...
  // Sensitive strings (item field values, tokens): the schema member carries
  ...
  // names for Connect), so no runtime case discrimination is needed.
  ...
  operationDecl: {
    contextType: "OnepasswordOpContext",
    commonErrorType: "OnepasswordOpError",
    commonErrorClasses: ["UnknownOnepasswordError"],
    protocol: "OnepasswordProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/spec-mirror-onepassword)",
