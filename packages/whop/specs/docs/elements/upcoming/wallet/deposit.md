...
Mounts inside [`Wallet`](/elements/upcoming/wallet/overview). Pass props and callbacks through the create options or React props. Keep the created handle, or React `ref`, to call `showStep()` and `refresh()`.
...
The viewer chose bank transfer before this account has a virtual bank account. The element opens identity verification itself — this fires alongside, so a host can record the trip.
