 ...
 * Effect SDK.
 *
 * Input:  .generated-specs/okta.json  (written by scripts/convert.ts)
 * Output: src/services/okta.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is Okta's provider spec.
 * Okta keeps the wire's camelCase member names on the TS surface, so no
...
/** Okta's provider spec for the shared smithy→SDK compiler. */
const oktaSpec: SdkSpec = {
  ...
  // Sensitive strings (API tokens, client secrets, passwords): the schema
  ...
  // names for Okta), so no runtime case discrimination is needed.
  union: ({ name, caseTargets, tsRef }) => [
    `export type ${name} = ${caseTargets.map(tsRef).join(" | ") || "unknown"};`,
    `export const ${name} = /*@__PURE__*/ S.Unknown as any as S.Schema<${name}>;\n`,
  ],

  // Converter-stamped pagination: cursor (`after` / next) when the spec
  // carries it. Okta list endpoints also page via the `Link` response
  // header; those stay plain operations unless the converter stamps a trait.
  paginationProfiles: {
    cursor: {
  ...
  operationDecl: {
    contextType: "OktaOpContext",
    commonErrorType: "OktaOpError",
    commonErrorClasses: ["UnknownOktaError"],
    protocol: "OktaProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/spec-mirror-okta/specs/management.json)",
