/**
 * Turbopuffer retry configuration.
 *
 * Defines the per-SDK `Retry` Context.Service tag that
 * `packages/turbopuffer/src/client.ts` wires into `makeAPI`. Callers can
 * install a blanket retry policy at the layer level and have every
 * Turbopuffer API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Turbopuffer from "@distilled.cloud/turbopuffer";
 *
 * myEffect.pipe(Turbopuffer.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Turbopuffer.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Turbopuffer API calls. */
export class Retry extends Context.Service<Retry, Policy>()(
  "TurbopufferRetry",
) {}

/** Provides a custom retry policy to every Turbopuffer API call below it. */
