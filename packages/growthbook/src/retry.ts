/**
 * GrowthBook retry configuration.
 *
 ...
 * layer level and have every GrowthBook API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as GrowthBook from "@distilled.cloud/growthbook";
 *
 * myEffect.pipe(GrowthBook.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(GrowthBook.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of GrowthBook API calls. */
export class Retry extends Context.Service<Retry, Policy>()(
  "GrowthBookRetry",
) {}

/** Provides a custom retry policy to every GrowthBook API call below it. */
