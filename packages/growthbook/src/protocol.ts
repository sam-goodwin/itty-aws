/**
 * GrowthBookProtocol — hand-written.
 *
 * GrowthBook speaks plain bearer-authenticated JSON REST (no response
 * envelope), so the whole protocol is one `makeRestProtocol` call from
 * `core/protocol-rest`:
 *
 *   request:  credentials → `Authorization: Bearer <apiKey>` + base URL
 *             (default https://api.growthbook.io/api), resolved from the
 ...
 *             HTTP-status classes, then {@link UnknownGrowthBookError}.
 */
 ...
 * GrowthBookOpError, GrowthBookOpContext>` explicitly so the compiler never
 * infers these back out of the schema generics.
 */
export type GrowthBookOpError =
...
export const GrowthBookProtocol: Layer.Layer<API.Protocol> =
  makeRestProtocol<Config>({
    ...
    // GrowthBook's error body is `{ message: string }` — the factory's
    // default lenient envelope covers `{ code, message }`.
    unknownError: ({ code, message, body }) =>
      new UnknownGrowthBookError({
        code:
          typeof code === "string"
            ? code
            : code !== undefined
