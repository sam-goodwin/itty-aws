 ...
 * Effect SDK.
 *
 * Input:  .generated-specs/doppler.json  (written by scripts/convert.ts)
 * Output: src/services/doppler.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is Doppler's provider spec.
 * Doppler keeps the wire's snake_case member names on the TS surface, so no
...
const dopplerSpec: SdkSpec = {
  ...
  // Sensitive strings (tokens, secrets, API keys): the schema member
  ...
  // names for Doppler), so no runtime case discrimination is needed.
  union: ({ name, caseTargets, tsRef }) => [
    `export type ${name} = ${caseTargets.map(tsRef).join(" | ") || "unknown"};`,
    `export const ${name} = /*@__PURE__*/ S.Unknown as any as S.Schema<${name}>;\n`,
  ],

  // One pagination profile: Doppler's page/per_page lists are not auto-
  // detected by the converter; cursor-mode ops (if stamped) use core's
  // paginateCursor.
  paginationProfiles: {
    cursor: {
  ...
  operationDecl: {
    contextType: "DopplerOpContext",
    commonErrorType: "DopplerOpError",
