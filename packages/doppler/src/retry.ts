 ...
 * layer level and have every Doppler API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Doppler from "@distilled.cloud/doppler";
 *
 * myEffect.pipe(Doppler.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Doppler.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Doppler API calls. */
export class Retry extends Context.Service<Retry, Policy>()("DopplerRetry") {}

/** Provides a custom retry policy to every Doppler API call below it. */
