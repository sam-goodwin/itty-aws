/**
 * Chronosphere retry configuration.
 *
 ...
 * layer level and have every Chronosphere API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Chronosphere from "@distilled.cloud/chronosphere";
 *
 * myEffect.pipe(Chronosphere.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Chronosphere.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Chronosphere API calls. */
export class Retry extends Context.Service<Retry, Policy>()(
  "ChronosphereRetry",
) {}

/** Provides a custom retry policy to every Chronosphere API call below it. */
