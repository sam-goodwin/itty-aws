 ...
 * layer level and have every Zendesk API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Zendesk from "@distilled.cloud/zendesk";
 *
 * myEffect.pipe(Zendesk.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Zendesk.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Zendesk API calls. */
export class Retry extends Context.Service<Retry, Policy>()("ZendeskRetry") {}

/** Provides a custom retry policy to every Zendesk API call below it. */
