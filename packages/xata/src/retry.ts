 ...
 * layer level and have every Xata API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Xata from "@distilled.cloud/xata";
 *
 * myEffect.pipe(Xata.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Xata.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Xata API calls. */
export class Retry extends Context.Service<Retry, Policy>()("XataRetry") {}

/** Provides a custom retry policy to every Xata API call below it. */
