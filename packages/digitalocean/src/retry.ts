/**
 * DigitalOcean retry configuration.
 *
 ...
 * policy at the layer level and have every DigitalOcean API call below it
 * pick it up:
 *
 * @example
 * ```ts
 ...
 *
 * myEffect.pipe(DigitalOcean.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(DigitalOcean.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of DigitalOcean API calls. */
export class Retry extends Context.Service<Retry, Policy>()(
  "DigitalOceanRetry",
) {}

/** Provides a custom retry policy to every DigitalOcean API call below it. */
