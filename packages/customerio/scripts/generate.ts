 ...
 * Customer.io Effect SDK.
 *
 * Input:  .generated-specs/*.json  (written by scripts/convert.ts)
 * Output: src/services/*.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is Customer.io's provider spec.
 * Customer.io keeps the wire's snake_case member names on the TS surface, so
...
/** Customer.io's provider spec for the shared smithy→SDK compiler. */
const customerioSpec: SdkSpec = {
  ...
  // names for Customer.io), so no runtime case discrimination is needed.
  ...
  operationDecl: {
    contextType: "CustomerioOpContext",
    commonErrorType: "CustomerioOpError",
    commonErrorClasses: ["UnknownCustomerioError"],
    protocol: "CustomerioProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/spec-mirror-customerio)",
...
runGeneratorCli({
  description: "Generate the Customer.io Effect SDK from the Smithy model",
