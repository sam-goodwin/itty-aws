/**
 * Customer.io retry configuration.
 *
 ...
 * layer level and have every Customer.io API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Customerio from "@distilled.cloud/customerio";
 *
 * myEffect.pipe(Customerio.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Customerio.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Customer.io API calls. */
export class Retry extends Context.Service<Retry, Policy>()(
  "CustomerioRetry",
) {}

/** Provides a custom retry policy to every Customer.io API call below it. */
