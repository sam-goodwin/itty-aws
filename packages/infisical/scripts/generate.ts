#!/usr/bin/env bun
/**
 * generate — turn the Smithy JSON model in .generated-specs into the Infisical
 * Effect SDK.
 *
 * Input:  .generated-specs/infisical.json  (written by scripts/convert.ts)
 * Output: src/services/infisical.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is Infisical's provider spec.
 * Infisical keeps the wire's snake_case member names on the TS surface, so no
...
/** Infisical's provider spec for the shared smithy→SDK compiler. */
const infisicalSpec: SdkSpec = {
  ...
  // names for Infisical), so no runtime case discrimination is needed.
  ...
  operationDecl: {
    contextType: "InfisicalOpContext",
    commonErrorType: "InfisicalOpError",
    commonErrorClasses: ["UnknownInfisicalError"],
    protocol: "InfisicalProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/distilled-spec-infisical)",
...
runGeneratorCli({
  description: "Generate the Infisical Effect SDK from the Smithy model",
