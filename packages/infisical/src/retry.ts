 ...
 * layer level and have every Infisical API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Infisical from "@distilled.cloud/infisical";
 *
 * myEffect.pipe(Infisical.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Infisical.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Infisical API calls. */
export class Retry extends Context.Service<Retry, Policy>()("InfisicalRetry") {}

/** Provides a custom retry policy to every Infisical API call below it. */
