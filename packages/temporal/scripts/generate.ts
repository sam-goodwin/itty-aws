 ...
 * Temporal keeps the wire's camelCase member names on the TS surface, so no
...
/** Temporal's provider spec for the shared smithy→SDK compiler. */
const temporalSpec: SdkSpec = {
  ...
  // names for Temporal), so no runtime case discrimination is needed.
  ...
  operationDecl: {
    contextType: "TemporalOpContext",
    commonErrorType: "TemporalOpError",
    commonErrorClasses: ["UnknownTemporalError"],
    protocol: "TemporalProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/spec-mirror-temporal)",
...
runGeneratorCli({
  description: "Generate the Temporal Effect SDK from the Smithy model",
