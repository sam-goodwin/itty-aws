 ...
 * layer level and have every Temporal API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Temporal from "@distilled.cloud/temporal";
 *
 * myEffect.pipe(Temporal.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Temporal.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Temporal API calls. */
export class Retry extends Context.Service<Retry, Policy>()("TemporalRetry") {}

/** Provides a custom retry policy to every Temporal API call below it. */
