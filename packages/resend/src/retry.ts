 ...
 * layer level and have every Resend API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Resend from "@distilled.cloud/resend";
 *
 * myEffect.pipe(Resend.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Resend.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Resend API calls. */
export class Retry extends Context.Service<Retry, Policy>()("ResendRetry") {}

/** Provides a custom retry policy to every Resend API call below it. */
