...
import { getDomain } from "../src/operations/getDomain";
...
describe("getDomain", () => {
  it("retrieves a domain by id", async () => {
    const name = `distilled-resend-getDomain-${testRunId}.example.com`;
    ...
    const result = await runEffect(
      Effect.gen(function* () {
        ...
        return yield* getDomain({ domain_id: createdId as string });
      }).pipe(
        Effect.ensuring(
          Effect.suspend(() =>
            createdId
  ...
  it("fails with NotFound for a non-existent domain id", async () => {
    const error = await runEffect(
      getDomain({ domain_id: NON_EXISTENT_DOMAIN_ID }).pipe(Effect.flip),
    );
