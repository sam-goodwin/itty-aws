 ...
 * Effect SDK.
 *
 * Input:  .generated-specs/gusto.json  (written by scripts/convert.ts)
 * Output: src/services/gusto.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is Gusto's provider spec.
 * Gusto keeps the wire's snake_case member names on the TS surface, so no
...
/** Gusto's provider spec for the shared smithy→SDK compiler. */
const gustoSpec: SdkSpec = {
  ...
  // names for Gusto), so no runtime case discrimination is needed.
  ...
  operationDecl: {
    contextType: "GustoOpContext",
    commonErrorType: "GustoOpError",
    commonErrorClasses: ["UnknownGustoError"],
    protocol: "GustoProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/distilled-spec-gusto)",
