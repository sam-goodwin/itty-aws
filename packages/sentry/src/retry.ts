 ...
 * layer level and have every Sentry API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Sentry from "@distilled.cloud/sentry";
 *
 * myEffect.pipe(Sentry.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Sentry.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Sentry API calls. */
export class Retry extends Context.Service<Retry, Policy>()("SentryRetry") {}

/** Provides a custom retry policy to every Sentry API call below it. */
