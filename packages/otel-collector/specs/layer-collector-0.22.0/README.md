...
`../../patches/layer-collector-0.22.0/`.

## What this set is

The exact component set of the AWS-managed Lambda collector extension release
`layer-collector/0.22.0` (open-telemetry/opentelemetry-lambda) — the build tag
this directory is named after. Its default build pins:
...
The pins above are also machine-readable in `build.json`, which the generator
reads and stamps into `src/layer-collector-0.22.0/manifest.ts`. Because the set
is a property of the BUILD rather than of any one of those three module
versions, the build tag is the directory key — a future build that bumps only
contrib gets its own directory, not an overwrite of this one.
