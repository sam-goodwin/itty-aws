 ...
 * layer level and have every Render API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Render from "@distilled.cloud/render";
 *
 * myEffect.pipe(Render.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Render.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Render API calls. */
export class Retry extends Context.Service<Retry, Policy>()("RenderRetry") {}

/** Provides a custom retry policy to every Render API call below it. */
