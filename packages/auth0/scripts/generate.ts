#!/usr/bin/env bun
/**
 * generate — turn the Smithy JSON model in .generated-specs into the Auth0
 * Effect SDK.
 *
 * Input:  .generated-specs/auth0.json  (written by scripts/convert.ts)
 * Output: src/services/auth0.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is Auth0's provider spec.
 * Auth0 keeps the wire's camelCase member names on the TS surface, so no
...
/** Auth0's provider spec for the shared smithy→SDK compiler. */
const auth0Spec: SdkSpec = {
  ...
  // names for Auth0), so no runtime case discrimination is needed.
  union: ({ name, caseTargets, tsRef }) => [
    `export type ${name} = ${caseTargets.map(tsRef).join(" | ") || "unknown"};`,
    `export const ${name} = /*@__PURE__*/ S.Unknown as any as S.Schema<${name}>;\n`,
  ],

  // Auth0 list endpoints page with `page`/`per_page`; some also expose
  // cursor-style `from`/`take`. Converter-stamped traits pick a profile.
  paginationProfiles: {
    cursor: {
  ...
  operationDecl: {
    contextType: "Auth0OpContext",
    commonErrorType: "Auth0OpError",
    commonErrorClasses: ["UnknownAuth0Error"],
    protocol: "Auth0Protocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/spec-mirror-auth0/specs/openapi.json)",
...
runGeneratorCli({
  description: "Generate the Auth0 Effect SDK from the Smithy model",
