/**
 * better-auth retry configuration.
 *
 * Defines the per-SDK `Retry` Context.Service tag that
 * `packages/better-auth/src/client.ts` wires into `makeAPI`. Callers can
 * install a blanket retry policy at the layer level and have every
 * better-auth API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as BetterAuth from "@distilled.cloud/better-auth";
 *
 * myEffect.pipe(BetterAuth.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(BetterAuth.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of better-auth API calls. */
export class Retry extends Context.Service<Retry, Policy>()(
  "BetterAuthRetry",
) {}

/** Provides a custom retry policy to every better-auth API call below it. */
