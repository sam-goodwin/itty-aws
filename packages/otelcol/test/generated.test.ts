...
import { BatchProcessor } from "../src/0.151.0/processors/batch.ts";
import { OtlpHttpExporter } from "../src/0.151.0/exporters/otlphttp.ts";
import { DebugExporter } from "../src/0.151.0/exporters/debug.ts";
import { OtlpReceiver } from "../src/0.151.0/receivers/otlp.ts";
import { components, version } from "../src/0.151.0/manifest.ts";
...
describe("patched reflector artifacts", () => {
  ...
  it("types debug verbosity as the config-file enum, not an integer", () => {
    ...
    expect(() => encode(DebugExporter)({ verbosity: 2 })).toThrowError();
  });
