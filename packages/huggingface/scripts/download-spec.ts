#!/usr/bin/env bun
/**
 * Download Hugging Face's OpenAPI description.
 *
 * The Hub publishes ONE OpenAPI 3.1 document at
 * https://huggingface.co/.well-known/openapi.json — no git repo, no release
 * tags, no versioned URL. There is nothing to add as a submodule (the way
 * `github`/`stripe`/`neon` vendor their spec repos), so the spec is downloaded
 * and COMMITTED under `specs/`, exactly as vercel and cloudflare commit their
 * downloaded API documents. Regenerating is then a two-step:
 *
 ...
 *   bun scripts/download-spec.ts https://huggingface.co/.well-known/openapi.json
 */
...
const SPEC_URL = "https://huggingface.co/.well-known/openapi.json";
    ...
    try: async () => {
      const res = await fetch(url, {
        headers: {
          ...
          "user-agent": "distilled.cloud-huggingface-spec-downloader",
        },
      });
      if (!res.ok) {
...
const downloadSpec = Command.make(
  "download-spec",
  {
    specUrl: Argument.string("spec-url").pipe(
      ...
      Argument.withDescription(
        "The OpenAPI document to download (default: the Hub's published spec)",
      ),
    ),
    out: Flag.string("out").pipe(
      Flag.withDefault("specs/openapi.json"),
      Flag.withDescription(
        "Where to write the spec (relative to the huggingface package root)",
      ),
    ),
  },
  (config) =>
    Effect.gen(function* () {
      ...
      // The huggingface/ folder is the parent of this scripts/ dir.
      const root = path.resolve(import.meta.dir, "..");
      const outPath = path.resolve(root, config.out);

      yield* Console.log("🤗 Hugging Face OpenAPI spec downloader");
  ...
  Command.withDescription(
    "Download the Hugging Face Hub's published OpenAPI document into ./specs",
  ),
);
