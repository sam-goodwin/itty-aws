 ...
 * Hugging Face calls below it and core's `makeDefault` is the fallback when
 * none is provided.
 *
 * The Hub answers a tripped rate limit with `429` plus `Retry-After`; the
 * REST protocol stamps that onto the retryable error classes, and the default
 ...
 *
 * myEffect.pipe(HuggingFace.Retry.transient);
 * ```
 */
