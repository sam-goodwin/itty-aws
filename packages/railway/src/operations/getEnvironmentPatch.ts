...
export const GetEnvironmentPatchInput = Schema.Struct({
  id: Schema.String,
}).pipe(
  T.Http({ method: "POST", path: "/graphql/v2" }),
  T.GraphQLOp({
    query: __document,
    operationName: "getEnvironmentPatch",
    type: "query",
  }),
);
export type GetEnvironmentPatchInput = typeof GetEnvironmentPatchInput.Type;

// Output Schema (GraphQL selection set)
export const GetEnvironmentPatchOutput = Schema.Struct({
...
export type GetEnvironmentPatchOutput = typeof GetEnvironmentPatchOutput.Type;

/**
 * Get a single environment patch by ID
 */
export const getEnvironmentPatch = API.make(() => ({
  inputSchema: GetEnvironmentPatchInput,
  outputSchema: GetEnvironmentPatchOutput,
}));
