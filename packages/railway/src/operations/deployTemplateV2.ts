...
export type DeployTemplateV2Input = typeof DeployTemplateV2Input.Type;

// Output Schema (GraphQL selection set)
export const DeployTemplateV2Output = Schema.Struct({
  projectId: Schema.String,
  workflowId: Schema.NullOr(Schema.String),
}).pipe(T.ResponsePath("templateDeployV2"));
export type DeployTemplateV2Output = typeof DeployTemplateV2Output.Type;

/**
 * Deploys a template using the serialized template config
 */
export const deployTemplateV2 = API.make(() => ({
  inputSchema: DeployTemplateV2Input,
  outputSchema: DeployTemplateV2Output,
}));
