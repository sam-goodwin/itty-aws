 ...
 * Railway API client.
 *
 * Railway exposes a single GraphQL endpoint at `${apiBaseUrl}/graphql/v2`
 * (default `https://backboard.railway.com`). All operations are dispatched
 * as `POST /graphql/v2` with a `{ query, operationName, variables }`
 * envelope — the shared client takes care of that wrapping when an
 * operation's input schema carries a `T.GraphQLOp` annotation (see
 * `scripts/generate.ts`).
 *
 ...
 *    Each error has `message` and optional `extensions.code`. The shared
 *    client detects this case and routes the envelope through `matchError`.
 *
 * `matchError` first checks the GraphQL `extensions.code` against the typed
 * `RAILWAY_ERROR_CODE_MAP`, then falls back to the HTTP status map, and
 * finally to `UnknownRailwayError`.
 */
...
export { UnknownRailwayError } from "./errors.ts";
import { Credentials } from "./credentials.ts";

/**
 * Single GraphQL error in the `errors[]` array of a response envelope.
 * Mirrors the shape produced by graphql-js and used by Railway's backboard.
 */
...
/**
 * Match a Railway error response (GraphQL envelope or HTTP-level body) to
...
const matchError = (
  ...
  return Effect.fail(new UnknownRailwayError({ body: errorBody }));
};
