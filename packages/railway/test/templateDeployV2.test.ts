...
describe("templateDeployV2", () => {
  it(
    "happy path - exercises the API and surfaces a typed RailwayInvalidInput when the referenced template config is not deployable",
    async () => {
  ...
  it(
    "error - RailwayInvalidInput for an empty templateId",
    async () => {
      const error = await runEffect(
        templateDeployV2({
          input: {
