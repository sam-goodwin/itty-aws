...
describe("twoFactorInfoValidate", () => {
  it(
    "happy path - exercises 2FA validation with a fabricated TOTP code (returns RailwayInvalidInput because no matching authenticator secret is enrolled)",
    async () => {
  ...
  it(
    "error - RailwayInvalidInput for an empty TOTP token",
    async () => {
      const error = await runEffect(
