...
describe("service", () => {
  it(
    "happy path - returns service details for a real service id",
    async () => {
      await runEffect(
        Effect.gen(function* () {
          ...
          for (const edge of projectsPage.edges) {
            ...
            const found = deps.edges.find((e) => e.node.serviceId !== null)?.node.serviceId;
            if (found) {
              serviceId = found;
              break;
            }
          ...
          expect(typeof result.hasHiddenRegistryCredentialsFromTemplate).toBe("boolean");
          ...
          expect(["free", "hobby", "pro", "trial"]).toContain(result.project.subscriptionType);
          if (result.project.workspace !== null) {
            ...
            expect(["FREE", "TEAM", "USER"]).toContain(result.project.workspace.subscriptionModel);
          }
        }),
      );
    },
