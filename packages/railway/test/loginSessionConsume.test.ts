...
describe("loginSessionConsume", () => {
  it(
    "happy path - exercises the API and surfaces a typed RailwayNotFound for a non-existent login-session code",
    async () => {
  ...
  it(
    "error - RailwayNotFound for a non-existent login-session code",
    async () => {
      const error = await runEffect(
        loginSessionConsume({ code: NON_EXISTENT_CODE }).pipe(Effect.flip),
      );
