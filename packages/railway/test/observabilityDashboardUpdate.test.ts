...
describe("observabilityDashboardUpdate", () => {
  it(
    "happy path - exercises the API and surfaces a typed RailwayNotFound for a non-existent observability-dashboard id",
    async () => {
  ...
  it(
    "error - RailwayNotFound for a non-existent observability-dashboard id",
    async () => {
      const error = await runEffect(
        observabilityDashboardUpdate({
          id: NON_EXISTENT_UUID,
  ...
  it(
    "error - RailwayInvalidInput for an empty dashboard id",
    async () => {
      const error = await runEffect(
        observabilityDashboardUpdate({
          id: "",
