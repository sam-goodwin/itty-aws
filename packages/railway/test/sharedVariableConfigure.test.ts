...
describe("sharedVariableConfigure", () => {
  it(
    "happy path - exercises the API and surfaces a typed RailwayInvalidInput when the referenced shared variable does not exist",
    async () => {
  ...
  it(
    "error - RailwayInvalidInput for an empty variable name",
    async () => {
      const error = await runEffect(
        sharedVariableConfigure({
          input: {
