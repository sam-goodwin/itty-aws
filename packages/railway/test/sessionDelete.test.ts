...
describe("sessionDelete", () => {
  it(
    "happy path - exercises the API and surfaces a typed RailwayNotFound when the referenced session does not exist",
    async () => {
  ...
  it(
    "error - RailwayNotFound for a non-existent session id",
    async () => {
      const error = await runEffect(
        sessionDelete({ id: NON_EXISTENT_UUID }).pipe(Effect.flip),
      );
