...
describe("serviceDomainDelete", () => {
  it("happy path - deletes a service domain that was just created", async () => {
    void testRunId;
    await runEffect(
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          const result = yield* serviceDomainDelete({ id: created.id });
  ...
  it("error - RailwayNotAuthorized when bearer token is invalid", async () => {
    ...
    const error = await Effect.runPromise(
      serviceDomainDelete({ id: NON_EXISTENT_UUID }).pipe(
        Effect.flip,
  ...
  it("error - RailwayNotFound for a non-existent service domain id", async () => {
    const error = await runEffect(
      serviceDomainDelete({ id: NON_EXISTENT_UUID }).pipe(Effect.flip),
    );
