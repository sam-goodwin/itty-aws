...
describe("tcpProxies", () => {
  it(
    "happy path - lists TCP proxies for a real (environment, service) pair",
    async () => {
      await runEffect(
        Effect.gen(function* () {
          ...
          for (const proxy of result) {
            ...
            expect(["ACTIVE", "CREATING", "DELETED", "DELETING", "UNSPECIFIED", "UPDATING"]).toContain(proxy.syncStatus);
