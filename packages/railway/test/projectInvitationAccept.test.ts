...
describe("projectInvitationAccept", () => {
  it(
    "happy path - exercises the API and surfaces a typed RailwayNotFound when the invitation code does not exist",
    async () => {
  ...
  it(
    "error - RailwayNotFound for a non-existent invitation code",
    async () => {
      const error = await runEffect(
        projectInvitationAccept({
