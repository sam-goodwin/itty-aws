...
describe("templateSourceForProject", () => {
  it(
    "happy path - returns template source (or null) for a real project",
    async () => {
      ...
      await runEffect(
        Effect.gen(function* () {
          ...
          return yield* Effect.gen(function* () {
            const result = yield* templateSourceForProject({ projectId: project.id });
          ...
          }).pipe(Effect.ensuring(projectDelete({ id: project.id }).pipe(Effect.ignore)));
        }),
      );
    },
    60_000,
  ...
  it(
    ...
    async () => {
      const error = await runEffect(
        templateSourceForProject({ projectId: NON_EXISTENT_UUID }).pipe(Effect.flip),
      );
