...
describe("upsertSlackChannel", () => {
  it("happy path - invokes Slack channel upsert for the authenticated workspace (returns true if Slack integration is configured, RailwayInvalidInput otherwise)", async () => {
    await runEffect(
      Effect.gen(function* () {
  ...
  it("error - RailwayInvalidInput for a non-existent workspace id", async () => {
    const error = await runEffect(
      upsertSlackChannel({ workspaceId: NON_EXISTENT_UUID }).pipe(Effect.flip),
    );
