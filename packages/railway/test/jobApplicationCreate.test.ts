...
describe("jobApplicationCreate", () => {
  it(
    "happy path - exercises the API and surfaces a typed RailwayInvalidInput when the referenced job does not exist",
    async () => {
  ...
  it(
    "error - RailwayInvalidInput for an empty jobId",
    async () => {
      const error = await runEffect(
        jobApplicationCreate({
          input: {
