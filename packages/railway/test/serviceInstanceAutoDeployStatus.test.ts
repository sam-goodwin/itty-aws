...
describe("serviceInstanceAutoDeployStatus", () => {
  it(
    "happy path - returns auto-deploy status for a real (project, environment, service) triple",
    async () => {
      await runEffect(
        Effect.gen(function* () {
  ...
  it(
    "error - RailwayNotFound for a non-existent (project, environment, service) triple",
    async () => {
