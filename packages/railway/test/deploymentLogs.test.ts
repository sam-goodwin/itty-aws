...
import { deploymentLogs } from "../src/operations/deploymentLogs.ts";
...
describe("deploymentLogs", () => {
  it("happy path - returns log entries for an existing deployment", async () => {
    const result = await runEffect(
      Effect.gen(function* () {
        ...
        const logs = yield* deploymentLogs({
  ...
  it("error - RailwayNotAuthorized when bearer token is invalid", async () => {
    ...
    const error = await Effect.runPromise(
      deploymentLogs({ deploymentId: NON_EXISTENT_UUID, limit: 10 }).pipe(
        Effect.flip,
