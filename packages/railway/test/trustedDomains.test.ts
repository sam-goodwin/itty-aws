...
describe("trustedDomains", () => {
  it(
    "happy path - lists trusted domains for the authenticated user's workspace",
    async () => {
      const result = await runEffect(
        Effect.gen(function* () {
          ...
          if (!workspaceId) {
            return { edges: [], pageInfo: { endCursor: null, hasNextPage: false, hasPreviousPage: false, startCursor: null } };
          }
