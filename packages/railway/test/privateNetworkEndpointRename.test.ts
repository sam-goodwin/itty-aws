...
describe("privateNetworkEndpointRename", () => {
  it(
    "happy path - exercises the API and surfaces a typed RailwayNotFound when the referenced endpoint/network does not exist",
    async () => {
  ...
  it(
    "error - RailwayInvalidInput for an empty endpoint id",
    async () => {
      const error = await runEffect(
        privateNetworkEndpointRename({
