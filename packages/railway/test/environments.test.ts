...
describe("environments", () => {
  it("happy path - returns paginated environments for the shared project", async () => {
    const project = await getSharedProject();

    const envs = await runEffect(
      environments({ projectId: project.id, first: 10 }),
    );
  ...
  it("error - RailwayNotAuthorized when bearer token is invalid", async () => {
    ...
    const error = await Effect.runPromise(
      environments({ projectId: NON_EXISTENT_UUID, first: 5 }).pipe(
        Effect.flip,
  ...
  it("error - non-existent projectId surfaces RailwayNotAuthorized", async () => {
    // Railway returns "Not Authorized" (not "not found") for resources
    // outside the caller's workspace — its resolvers can't tell missing
    // from forbidden when the bearer is valid.
