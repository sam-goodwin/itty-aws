...
describe("workspacePermissionChange", () => {
  it("happy path - exercises workspace permission change with fabricated workspaceId/userId (real promotion/demotion would alter the test runner's own role on its workspace and could break subsequent tests; lands in RailwayNotFound for the fabricated workspace)", async () => {
  ...
  it("error - RailwayInvalidInput for a malformed workspace/user id", async () => {
    const error = await runEffect(
      workspacePermissionChange({
        input: {
          role: "MEMBER",
