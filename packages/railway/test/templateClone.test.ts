...
describe("templateClone", () => {
  it(
    "happy path - exercises the API and surfaces a typed RailwayNotFound when the referenced template does not exist",
    async () => {
  ...
  it(
    "error - RailwayNotFound for a non-existent template code",
    async () => {
      const error = await runEffect(
        templateClone({
          input: {
  ...
  it(
    "error - RailwayInvalidInput for an empty template code",
    async () => {
      const error = await runEffect(
        templateClone({
          input: {
