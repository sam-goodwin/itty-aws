...
describe("observabilityDashboardReset", () => {
  it("happy path - exercises the API and surfaces a typed RailwayNotFound for a non-existent observability-dashboard id", async () => {
  ...
  it("error - RailwayNotFound for a non-existent observability-dashboard id", async () => {
    const error = await runEffect(
      observabilityDashboardReset({ id: NON_EXISTENT_UUID }).pipe(Effect.flip),
    );
    expect([
