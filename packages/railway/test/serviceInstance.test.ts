...
describe("serviceInstance", () => {
  it(
    "happy path - returns service instance for a real (environmentId, serviceId) pair",
    async () => {
      await runEffect(
        Effect.gen(function* () {
          ...
          expect(["HEROKU", "NIXPACKS", "PAKETO", "RAILPACK"]).toContain(result.builder);
          expect(["ALWAYS", "NEVER", "ON_FAILURE"]).toContain(result.restartPolicyType);
          ...
          expect(["free", "hobby", "pro", "trial"]).toContain(result.service.project.subscriptionType);

          if (result.latestDeployment !== null) {
            expect(typeof result.latestDeployment.id).toBe("string");
            expect(["BUILDING", "CRASHED", "DEPLOYING", "FAILED", "INITIALIZING", "NEEDS_APPROVAL", "QUEUED", "REMOVED", "REMOVING", "SKIPPED", "SLEEPING", "SUCCESS", "WAITING"]).toContain(result.latestDeployment.status);
          }
        }),
      );
    },
  ...
  it(
    "error - RailwayNotFound for non-existent (environmentId, serviceId)",
    async () => {
