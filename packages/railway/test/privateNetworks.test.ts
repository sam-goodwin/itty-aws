...
describe("privateNetworks", () => {
  it("happy path - lists private networks for a freshly created project's base environment", async () => {
    await runEffect(
      Effect.gen(function* () {
  ...
  it("error - non-existent environmentId surfaces RailwayNotAuthorized", async () => {
    const error = await runEffect(
      privateNetworks({ environmentId: NON_EXISTENT_UUID }).pipe(Effect.flip),
    );
