...
describe("projectTransferConfirm", () => {
  it(
    "happy path - exercises the API and surfaces a typed RailwayNotFound when the referenced ownership transfer does not exist",
    async () => {
  ...
  it(
    "error - RailwayNotFound for a non-existent ownershipTransferId",
    async () => {
      const error = await runEffect(
        projectTransferConfirm({
          input: {
