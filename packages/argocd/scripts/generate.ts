#!/usr/bin/env bun
/**
 * generate — turn the Smithy JSON model in .generated-specs into the Argo CD
 * Effect SDK.
 *
 * Input:  .generated-specs/argocd.json  (written by scripts/convert.ts)
 * Output: src/services/argocd.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is Argo CD's provider spec.
 * Argo CD keeps the wire's camelCase member names on the TS surface, so no
...
/** Argo CD's provider spec for the shared smithy→SDK compiler. */
const argocdSpec: SdkSpec = {
  ...
  // Sensitive strings (tokens, passwords, API keys): the schema
  ...
  // names for Argo CD), so no runtime case discrimination is needed.
  ...
  operationDecl: {
    contextType: "ArgocdOpContext",
    commonErrorType: "ArgocdOpError",
    commonErrorClasses: ["UnknownArgocdError"],
    protocol: "ArgocdProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/spec-mirror-argocd)",
...
runGeneratorCli({
  description: "Generate the Argo CD Effect SDK from the Smithy model",
