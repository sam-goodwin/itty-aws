/**
 * SpacetimeDB retry configuration.
 *
 ...
 * layer level and have every SpacetimeDB API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as SpacetimeDB from "@distilled.cloud/spacetimedb";
 *
 * myEffect.pipe(SpacetimeDB.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(SpacetimeDB.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of SpacetimeDB API calls. */
export class Retry extends Context.Service<Retry, Policy>()(
  "SpacetimeDBRetry",
) {}

/** Provides a custom retry policy to every SpacetimeDB API call below it. */
