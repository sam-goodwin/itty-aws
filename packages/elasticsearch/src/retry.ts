/**
 * Elasticsearch retry configuration.
 *
 ...
 * layer level and have every Elasticsearch API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Elasticsearch from "@distilled.cloud/elasticsearch";
 *
 * myEffect.pipe(Elasticsearch.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Elasticsearch.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Elasticsearch API calls. */
export class Retry extends Context.Service<Retry, Policy>()(
  "ElasticsearchRetry",
) {}

/** Provides a custom retry policy to every Elasticsearch API call below it. */
