 ...
 * SurrealDB Effect SDK.
 *
 * Input:  .generated-specs/surrealdb.json  (written by scripts/convert.ts)
 * Output: src/services/surrealdb.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is SurrealDB's provider spec.
 * SurrealDB keeps the wire's member names on the TS surface, so no member
...
/** SurrealDB's provider spec for the shared smithy→SDK compiler. */
const surrealdbSpec: SdkSpec = {
  ...
  // names for SurrealDB), so no runtime case discrimination is needed.
  ...
  operationDecl: {
    contextType: "SurrealdbOpContext",
    commonErrorType: "SurrealdbOpError",
    commonErrorClasses: ["UnknownSurrealdbError"],
    protocol: "SurrealdbProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/spec-mirror-surrealdb)",
...
runGeneratorCli({
  description: "Generate the SurrealDB Effect SDK from the Smithy model",
