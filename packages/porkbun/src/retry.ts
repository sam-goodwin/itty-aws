 ...
 * layer level and have every Porkbun API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Porkbun from "@distilled.cloud/porkbun";
 *
 * myEffect.pipe(Porkbun.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Porkbun.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Porkbun API calls. */
export class Retry extends Context.Service<Retry, Policy>()("PorkbunRetry") {}

/** Provides a custom retry policy to every Porkbun API call below it. */
