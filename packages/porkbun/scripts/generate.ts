#!/usr/bin/env bun
/**
 * generate — turn the Smithy JSON model in .generated-specs into the Porkbun
 * Effect SDK.
 *
 * Input:  .generated-specs/porkbun.json  (written by scripts/convert.ts)
 * Output: src/services/porkbun.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is Porkbun's provider spec.
 * Porkbun keeps the wire's member names on the TS surface, so no member
...
/** Porkbun's provider spec for the shared smithy→SDK compiler. */
const porkbunSpec: SdkSpec = {
  ...
  // names for Porkbun), so no runtime case discrimination is needed.
  ...
  operationDecl: {
    contextType: "PorkbunOpContext",
    commonErrorType: "PorkbunOpError",
    commonErrorClasses: ["UnknownPorkbunError"],
    protocol: "PorkbunProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/spec-mirror-porkbun)",
...
runGeneratorCli({
  description: "Generate the Porkbun Effect SDK from the Smithy model",
