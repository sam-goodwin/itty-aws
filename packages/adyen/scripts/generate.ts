#!/usr/bin/env bun
/**
 * generate — turn the Smithy JSON model in .generated-specs into the Adyen
 * Effect SDK.
 *
 * Input:  .generated-specs/adyen.json  (written by scripts/convert.ts)
 * Output: src/services/adyen.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is Adyen's provider spec.
 * Adyen keeps the wire's camelCase member names on the TS surface, so no
...
/** Adyen's provider spec for the shared smithy→SDK compiler. */
const adyenSpec: SdkSpec = {
  ...
  // names for Adyen), so no runtime case discrimination is needed.
  ...
  operationDecl: {
    contextType: "AdyenOpContext",
    commonErrorType: "AdyenOpError",
    commonErrorClasses: ["UnknownAdyenError"],
    protocol: "AdyenProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/spec-mirror-adyen)",
...
runGeneratorCli({
  description: "Generate the Adyen Effect SDK from the Smithy model",
