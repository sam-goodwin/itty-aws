...
// Input Schema
export interface SecretkeyGetInput {
  app_name: string;
  secret_name: string;
  min_version?: string;
}
...
) as unknown as Schema.Codec<SecretkeyGetInput>;

// Output Schema
export interface SecretkeyGetOutput {
  created_at?: string;
  name?: string;
  public_key?: number[];
  type?: string;
...
}) as unknown as Schema.Codec<SecretkeyGetOutput>;
