...
describe("MachinesStop", () => {
  ...
  beforeAll(async () => { skipApps = !(await canManageApps()); });
    ...
    const appName = `distilled-fly-mstop-${testRunId}`;

  it("happy path - stops a machine", async (ctx) => {
    if (skipApps) return ctx.skip();
    // Stopping a machine requires it to be running. With skip_launch
    // the machine is in created state, so accept success or a relevant error.
    await runEffect(
      Effect.gen(function* () {
