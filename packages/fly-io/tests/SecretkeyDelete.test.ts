...
describe("SecretkeyDelete", () => {
  ...
  beforeAll(async () => { skipApps = !(await canManageApps()); });
    ...
    const appName = `distilled-fly-skdel-${testRunId}`;

  it("happy path - delete nonexistent key (expects NotFound)", async (ctx) => {
  ...
  it("error - NotFound for non-existent key on valid app", async (ctx) => {
    if (skipApps) return ctx.skip();
    const validAppName = `distilled-fly-skdel2-${testRunId}`;
    await runEffect(
      Effect.gen(function* () {
