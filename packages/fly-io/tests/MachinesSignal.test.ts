...
describe("MachinesSignal", () => {
  ...
  beforeAll(async () => { skipApps = !(await canManageApps()); });
    ...
    const appName = `distilled-fly-msig-${testRunId}`;

  it("happy path - sends a signal to a machine", async (ctx) => {
    if (skipApps) return ctx.skip();
    // Sending a signal requires a running machine. With skip_launch the
    // machine won't be running, so accept success or a relevant error.
    await runEffect(
      Effect.gen(function* () {
