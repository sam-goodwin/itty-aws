...
describe("MachinesCordon", () => {
  ...
  it("happy path - cordons a machine", async (ctx) => {
    ...
    await runEffect(
      Effect.gen(function* () {
        ...
        // Cordon may succeed or fail depending on machine state;
        // accept success or a BadRequest (machine not in right state)
        const result = yield* MachinesCordon({
          app_name: appName,
  ...
  it("error - BadRequest for machine not in valid state", async (ctx) => {
    if (skipApps) return ctx.skip();
    // Cordoning a freshly created (not started) machine may produce BadRequest
    await runEffect(
      Effect.gen(function* () {
        yield* AppsCreate({ org_slug: "personal", name: appName });
        const machine = yield* MachinesCreate({
