...
describe("AppCertificatesCustomDelete", () => {
  ...
  const hostname = `${testRunId}.ccdel.example.com`;

  it("happy path - removes custom certificate from an existing cert", async (ctx) => {
    if (skipApps) return ctx.skip();
    await runEffect(
      Effect.gen(function* () {
        yield* AppsCreate({ org_slug: "personal", name: appName });
        // Create a cert via ACME first; deleting the custom portion on
        // a cert that has no custom component should still succeed or
        // return the cert state.
        ...
        if (result.ok) {
          ...
          // No custom cert to delete — may return NotFound or BadRequest
