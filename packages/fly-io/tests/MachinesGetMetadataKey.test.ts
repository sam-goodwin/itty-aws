...
import { MachinesGetMetadataKey } from "../src/operations/MachinesGetMetadataKey";
...
describe("MachinesGetMetadataKey", () => {
  let skipApps = false;
  beforeAll(async () => { skipApps = !(await canManageApps()); });

    const appName = `distilled-fly-mgmk-${testRunId}`;

  it("happy path - gets metadata key value", async (ctx) => {
    ...
    await runEffect(
      Effect.gen(function* () {
        ...
        const result = yield* MachinesGetMetadataKey({
          app_name: appName,
  ...
  it("error - NotFound for non-existent machine on valid app", async (ctx) => {
    ...
    await runEffect(
      Effect.gen(function* () {
        yield* AppsCreate({ org_slug: "personal", name: appName });
        const error = yield* MachinesGetMetadataKey({
          app_name: appName,
          machine_id: "0000000000000",
          key: "test-key",
        }).pipe(Effect.flip);
