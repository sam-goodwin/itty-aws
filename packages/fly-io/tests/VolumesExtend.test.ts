...
describe("VolumesExtend", () => {
  ...
  beforeAll(async () => { skipApps = !(await canManageApps()); });
    ...
    const appName = `distilled-fly-volext-${testRunId}`;

  it("happy path - extends a volume", async (ctx) => {
  ...
  it("error - BadRequest shrinking a volume", async (ctx) => {
    if (skipApps) return ctx.skip();
    await runEffect(
      Effect.gen(function* () {
        yield* AppsCreate({ org_slug: "personal", name: appName });
        const volume = yield* VolumesCreate({
          app_name: appName,
          name: "test_vol_shrink",
          region: "ewr",
          size_gb: 2,
        });
        // Attempting to shrink a volume should produce BadRequest
        const error = yield* VolumesExtend({
          app_name: appName,
          volume_id: volume.id!,
          size_gb: 1,
