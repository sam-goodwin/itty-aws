...
describe("MachinesSuspend", () => {
  ...
  beforeAll(async () => { skipApps = !(await canManageApps()); });
    ...
    const appName = `distilled-fly-msusp-${testRunId}`;

  it("happy path - suspends a machine", async (ctx) => {
    if (skipApps) return ctx.skip();
    // Suspending requires a running machine. With skip_launch the machine
