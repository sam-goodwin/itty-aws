...
describe("VolumesList", () => {
  ...
  beforeAll(async () => { skipApps = !(await canManageApps()); });
    ...
    const appName = `distilled-fly-volls-${testRunId}`;

  it("happy path - lists volumes for an app", async (ctx) => {
    if (skipApps) return ctx.skip();
    await runEffect(
      Effect.gen(function* () {
        yield* AppsCreate({ org_slug: "personal", name: appName });
        const result = yield* VolumesList({ app_name: appName });
  ...
  it("error - NotFound for non-existent app", async () => {
    await runEffect(
      VolumesList({ app_name: "nonexistent-app-00000000" }).pipe(
        Effect.flip,
        Effect.map((e) => {
