...
describe("MachinesShowLease", () => {
  ...
  beforeAll(async () => { skipApps = !(await canManageApps()); });
    ...
    const appName = `distilled-fly-mlease-${testRunId}`;

  it("happy path - retrieves lease for a machine", async (ctx) => {
    ...
    await runEffect(
      Effect.gen(function* () {
        ...
        // A freshly created machine may have no active lease, which could
        // return an empty/default response or NotFound for the lease.
        const result = yield* MachinesShowLease({
          app_name: appName,
        ...
        if (result.ok) {
          ...
          // No lease exists — may return NotFound
          expect(["NotFound", "UnknownFlyIoError"]).toContain(
