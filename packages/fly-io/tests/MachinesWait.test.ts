...
describe("MachinesWait", () => {
  ...
  beforeAll(async () => { skipApps = !(await canManageApps()); });
    ...
    const appName = `distilled-fly-mwait-${testRunId}`;

  it("happy path - waits for machine state", async (ctx) => {
    ...
    await runEffect(
      Effect.gen(function* () {
        ...
        // Wait for "stopped" state with a short timeout;
        // machine state may not match, so accept success or expected error
  ...
  it("error - BadRequest with invalid state expectations", async (ctx) => {
    if (skipApps) return ctx.skip();
    await runEffect(
      Effect.gen(function* () {
        yield* AppsCreate({ org_slug: "personal", name: appName });
        ...
        // Wait with state expectations that may produce BadRequest
        const result = yield* MachinesWait({
          app_name: appName,
          machine_id: machine.id!,
          state: "started",
