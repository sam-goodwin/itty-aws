...
describe("MachinesRestart", () => {
  ...
  beforeAll(async () => { skipApps = !(await canManageApps()); });
    ...
    const appName = `distilled-fly-mrest-${testRunId}`;

  it("happy path - restarts a machine", async (ctx) => {
    if (skipApps) return ctx.skip();
    // Restart requires a running machine. With skip_launch the machine
    // won't be running, so accept success or a relevant error.
    await runEffect(
      Effect.gen(function* () {
