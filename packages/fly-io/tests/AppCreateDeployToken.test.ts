...
describe("AppCreateDeployToken", () => {
  ...
  beforeAll(async () => { skipApps = !(await canManageApps()); });
    ...
    const appName = `distilled-fly-deptok-${testRunId}`;

  it("happy path - creates a deploy token for an app", async (ctx) => {
  ...
  it("error - BadRequest with invalid expiry", async (ctx) => {
    if (skipApps) return ctx.skip();
    await runEffect(
      Effect.gen(function* () {
        yield* AppsCreate({ org_slug: "personal", name: appName });
