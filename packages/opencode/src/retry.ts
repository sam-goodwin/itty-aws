 ...
 * layer level and have every OpenCode API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Opencode from "@distilled.cloud/opencode";
 *
 * myEffect.pipe(Opencode.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Opencode.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of OpenCode API calls. */
export class Retry extends Context.Service<Retry, Policy>()("OpencodeRetry") {}

/** Provides a custom retry policy to every OpenCode API call below it. */
