#!/usr/bin/env bun
/**
 * generate — turn the Smithy JSON model in .generated-specs into the OpenCode
 * Effect SDK.
 *
 * Input:  .generated-specs/opencode.json  (written by scripts/convert.ts)
 * Output: src/services/opencode.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is OpenCode's provider spec.
 * OpenCode keeps the wire's member names on the TS surface, so no member
...
/** OpenCode's provider spec for the shared smithy→SDK compiler. */
const opencodeSpec: SdkSpec = {
  ...
  // Sensitive strings (passwords, tokens): the schema member carries
  ...
  // names for OpenCode), so no runtime case discrimination is needed.
  ...
  operationDecl: {
    contextType: "OpencodeOpContext",
    commonErrorType: "OpencodeOpError",
    commonErrorClasses: ["UnknownOpencodeError"],
    protocol: "OpencodeProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/distilled-spec-opencode)",
...
runGeneratorCli({
  description: "Generate the OpenCode Effect SDK from the Smithy model",
