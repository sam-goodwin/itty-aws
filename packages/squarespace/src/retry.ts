/**
 * Squarespace retry configuration.
 *
 ...
 * layer level and have every Squarespace API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Squarespace from "@distilled.cloud/squarespace";
 *
 * myEffect.pipe(Squarespace.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Squarespace.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Squarespace API calls. */
export class Retry extends Context.Service<Retry, Policy>()(
  "SquarespaceRetry",
) {}

/** Provides a custom retry policy to every Squarespace API call below it. */
