 ...
 * Metabase Effect SDK.
 *
 * Input:  .generated-specs/metabase.json  (written by scripts/convert.ts)
 * Output: src/services/metabase.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is Metabase's provider spec.
 * Metabase keeps the wire's snake_case / kebab-case member names on the TS
...
/** Metabase's provider spec for the shared smithy→SDK compiler. */
const metabaseSpec: SdkSpec = {
  ...
  // Sensitive strings (API keys, passwords): the schema member carries
  ...
  operationDecl: {
    contextType: "MetabaseOpContext",
    commonErrorType: "MetabaseOpError",
    commonErrorClasses: ["UnknownMetabaseError"],
    protocol: "MetabaseProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/spec-mirror-metabase)",
...
runGeneratorCli({
  description: "Generate the Metabase Effect SDK from the Smithy model",
