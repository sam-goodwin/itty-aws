 ...
 * layer level and have every Metabase API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as Metabase from "@distilled.cloud/metabase";
 *
 * myEffect.pipe(Metabase.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Metabase.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Metabase API calls. */
export class Retry extends Context.Service<Retry, Policy>()("MetabaseRetry") {}

/** Provides a custom retry policy to every Metabase API call below it. */
