/**
 * Trigger.dev retry configuration.
 *
 ...
 * layer level and have every Trigger.dev API call below it pick it up:
 *
 * @example
 * ```ts
 * import * as TriggerDev from "@distilled.cloud/trigger-dev";
 *
 * myEffect.pipe(TriggerDev.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(TriggerDev.Retry.Retry, customPolicy));
 * ```
 */
...
/** Context tag for configuring retry behavior of Trigger.dev API calls. */
export class Retry extends Context.Service<Retry, Policy>()(
  "TriggerDevRetry",
) {}

/** Provides a custom retry policy to every Trigger.dev API call below it. */
