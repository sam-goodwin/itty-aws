 ...
 * Hostinger Effect SDK.
 *
 * Input:  .generated-specs/hostinger.json  (written by scripts/convert.ts)
 * Output: src/services/hostinger.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is Hostinger's provider spec.
 * Hostinger keeps the wire's snake_case member names on the TS surface, so
...
/** Hostinger's provider spec for the shared smithy→SDK compiler. */
const hostingerSpec: SdkSpec = {
  ...
  // Sensitive strings (API keys, passwords, tokens): the schema member
  ...
  // names for Hostinger), so no runtime case discrimination is needed.
  union: ({ name, caseTargets, tsRef }) => [
    `export type ${name} = ${caseTargets.map(tsRef).join(" | ") || "unknown"};`,
    `export const ${name} = /*@__PURE__*/ S.Unknown as any as S.Schema<${name}>;\n`,
  ],

  // Converter-stamped traits pick a profile. Hostinger list endpoints page
  // with `page`/`per_page` and `{ data, meta }`; some specs stamp cursor.
  paginationProfiles: {
    cursor: {
  ...
  operationDecl: {
    contextType: "HostingerOpContext",
    commonErrorType: "HostingerOpError",
    commonErrorClasses: ["UnknownHostingerError"],
    protocol: "HostingerProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/spec-mirror-hostinger)",
...
runGeneratorCli({
  description: "Generate the Hostinger Effect SDK from the Smithy model",
