...
describeLive("oauth2revokeToken", () => {
  it(
    "accepts a revoke request from a registered client (RFC 7009)",
    { timeout: 60_000 },
    async () => {
      const clientName = `distilled-oauth2-revoke-${testRunId}`;
  ...
  it(
    "rejects a revoke request with bogus client credentials",
    { timeout: 30_000 },
    async () => {
      const exit = await runEffect(
        Effect.exit(
  ...
  it(
    "rejects a revoke request missing client credentials",
    { timeout: 30_000 },
    async () => {
      const exit = await runEffect(
        Effect.exit(
