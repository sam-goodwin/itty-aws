 ...
 * `packages/polar/src/client.ts` wires into `makeAPI`. Callers can install a
 * blanket retry policy at the layer level and have every Polar API call below
 * it pick it up:
 *
 * @example
 * ```ts
 ...
 *
 * myEffect.pipe(Polar.Retry.transient);
 * Effect.provide(myEffect, Layer.succeed(Polar.Retry.Retry, customPolicy));
 * ```
 */
...
/** Provides a custom retry policy to every Polar API call below it. */
