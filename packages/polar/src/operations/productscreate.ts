...
export interface ProductscreateInput {
  ...
  recurring_interval_count?: unknown;
...
export const ProductscreateInput = /*@__PURE__*/ /*#__PURE__*/ Schema.Struct({
  ...
  meter_interval: Schema.optional(
    Schema.NullOr(Schema.Literals(["day", "week", "month", "year"])),
  ),
  meter_interval_count: Schema.optional(Schema.NullOr(Schema.Number)),
}).pipe(
  T.Http({ method: "POST", path: "/v1/products/" }),
) as unknown as Schema.Codec<ProductscreateInput>;

// Output Schema
export interface ProductscreateOutput {
  id: string;
  created_at: string;
...
}) as unknown as Schema.Codec<ProductscreateOutput>;
