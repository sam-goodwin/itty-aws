#!/usr/bin/env bun
/**
 * Fetches the Hetzner Cloud OpenAPI spec to ../specs/.
 *
 * Hetzner publishes ONE OpenAPI 3.1 document — the same file its own
 * documentation site renders. There is no git repo and no versioned URL, so the
 * mirror simply snapshots it.
 *
 * Usage:
...
const OPENAPI_SPEC_URL = "https://docs.hetzner.cloud/cloud.spec.json";
const SPECS_DIR = "../specs";
const OUTPUT_PATH = `${SPECS_DIR}/cloud.spec.json`;
...
async function main() {
  ...
  const response = await fetch(OPENAPI_SPEC_URL, {
    headers: {
      ...
      "user-agent": "distilled.cloud-hetzner-spec-mirror",
    },
  });
