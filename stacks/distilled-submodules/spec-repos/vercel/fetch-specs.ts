#!/usr/bin/env bun
/**
 * Fetches the Vercel OpenAPI spec to ../specs/.
 *
 * Vercel publishes ONE OpenAPI 3.0.3 document — no git repo, no release
 * tags, no versioned URL. The mirror snapshots it.
 *
 * Usage:
...
const OPENAPI_SPEC_URL = "https://openapi.vercel.sh/";
...
async function main() {
  ...
  const response = await fetch(OPENAPI_SPEC_URL, {
    headers: {
      ...
      "user-agent": "distilled.cloud-vercel-spec-mirror",
    },
  });
