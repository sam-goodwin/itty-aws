  ...
  switch (message.type) {
    case "system": {
      if ((message as any).subtype === "init") {
        ...
        console.log(`${DIM}Session:  ${m.session_id}${RESET}`);
        console.log(`${DIM}Model:    ${m.model}${RESET}`);
        console.log(`${DIM}Tools:    ${m.tools.join(", ")}${RESET}`);
        console.log(`${DIM}Auth:     ${m.apiKeySource ?? "unknown"}${RESET}`);
        console.log("");
      }
      break;
    }
    ...
    case "user": {
      ...
      if (msg.tool_use_result != null) {
        console.log(
          `${YELLOW}[tool_result]${RESET} ${DIM}${formatToolResult(msg.tool_use_result)}${RESET}`,
        );
      }
      break;
    }
    ...
    case "result": {
      ...
      console.log(`${GREEN}${BOLD}Result: ${m.subtype}${RESET}`);
      ...
      console.log(
        `\n${DIM}Duration: ${(m.duration_ms / 1000).toFixed(1)}s${RESET}`,
      );
      console.log(`${DIM}Cost:     $${m.total_cost_usd.toFixed(4)}${RESET}`);
      console.log(`${DIM}Turns:    ${m.num_turns}${RESET}`);
      break;
    }

    default: {
      const m = message as any;
      if (m.type === "stream_event") break;
      console.log(
